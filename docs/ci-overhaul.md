...
`merge_group` triggers on every gating workflow (queue runs select the full tier — a merge is validated on its exact merged result before landing, per Q4), a repo ruleset enabling the merge queue on main with `build_test result` as the required status, and a Monday 03:00 UTC scheduled full-tier run as a backstop. The install/mobile PR tiers landed with 5a. **Deferred**: asan/tsan/lsan need CMake wiring first — anira only wires RTSan (`ANIRA_WITH_RTSAN` → `tanh_add_sanitizer`); mirroring that for the others plus the suppression soak is library work (noengines scope per Q6 when it lands).
