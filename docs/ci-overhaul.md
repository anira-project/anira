...
**Status**: delivered through step 9(b)(c); step 9(a) and the sanitizer wiring remain · **Owner**: Valentin (M0 items 3+4), upstream coordination with Fares · **Date**: 2026-08-31
...
>
> **Status (2026-08-31, end of day)**: steps 0–8 and 9(b)(c) are delivered — both repos consume the ci-actions v0.3.x reusable workflows and shared mobile/wasm drivers, both mains are merge-queue-gated with `<name> result` contexts, coverage uploads to Codecov from pushes to main, and a 112-agent audit of the day's work ran clean apart from the fixes recorded below. Superseded detail from mid-flight follows unchanged: steps 0–6 are merged (5a: #134; 5b: on_tag on the shared actions per Q2, local composites deleted) — #129 (steps 0+1, incl. the resolved 0c: sccache spoke the retired v1 cache API without `ACTIONS_CACHE_SERVICE_V2=on`, every write failed read-only), #130 (diff-based clang-tidy), #131 (step 2, the preset catalog), ci-actions tagged `v0.1.0`→`v0.2.1` with tanh-lib pinned (tanh-lib#23). Steps 4–5 (#132, #134, 5b): build_test and every other workflow on the shared actions in preset mode, the PR/full tier split, MSVC under Ninja+vcvars (incl. native Windows-arm64), the gcc and clang-cl coverage legs, and the `build_test result` aggregation job — PR tier and full 23-leg tier validated green. Steps 5–7 follow; all open questions are resolved (§6), incl. the macOS consolidation (§3.6).
...
**Step 6 — merge queue + weekly full tier** *(anira, Valentin; needs 5; done in #137)*
`merge_group` triggers on every gating workflow (queue runs select the full tier — a merge is validated on its exact merged result before landing, per Q4) and a repo ruleset enabling the merge queue on main with `build_test result` as the required status. The install/mobile PR tiers landed with 5a. **The weekly cron was dropped by decision (2026-08-31)**: it is not in the roadmap mandate, and the merge queue already guarantees nothing lands without the full suite — its only residual value (quiet-week drift detection, cache re-seeding past the 7-day eviction) was judged not worth the machinery. Consequence to know: after an idle week the caches are cold and the first queue run pays the refetch, and environment drift (runner-image updates) surfaces in the first merge attempt after it happens. **Deferred**: asan/tsan/lsan need CMake wiring first — anira only wires RTSan (`ANIRA_WITH_RTSAN` → `tanh_add_sanitizer`); mirroring that for the others plus the suppression soak is library work (noengines scope per Q6 when it lands).

**Step 7 — mobile onto the shared actions** *(ci-actions + anira + tanh-lib; needs 1, and U5/U6 in a tagged ci-actions; combined with step 8 into one PR per repo, decided 2026-08-31)*
The decided end state (§3.7): extend `cmake-test-android`/`cmake-test-ios-simulator` with the U5 inputs and the U6 parser fix, tag, point anira's `build_test_mobile` at them, and delete the local emulator/simulator scripting. tanh-lib moves to the same tag in the same motion. Scope note: the *Android* driver converges (ci-actions v0.3.0 `cmake-test-android` carries staging via `DEVICE_DIR`/`EXTRA_PUSH_PATHS`/`RUN_ENV`, marker-based exit codes, failure collection; the caller contributes only its push list). anira's iOS job stays repo-local on purpose — it is the §3.6 consolidation (three backend sets sequentially on one simulator boot against the 5-concurrent macOS cap), which the one-preset-one-build-dir shape of the shared simulator action cannot express.

**Step 8 — tanh-lib convergence: one CI, zero duplicate code** *(tanh-lib + ci-actions; combined with step 7 — ci-actions PR #5 carries the reusable workflows, then one consumption PR per repo)*
tanh-lib adopts the identical setup — tier policy v2, merge queue, warm caches, result checks — and the de-duplication mechanism is the previously deferred **reusable-workflows consolidation**: the workflow *logic* (build-test with plan/tiers/result, warm-caches, the mobile runner drivers) moves into ci-actions as `workflow_call` reusable workflows, so anira and tanh-lib each keep only their *facts* — triggers, the matrix JSON, presets, repo-specific steps (models cache, linkage assertion, JUCE deps) — and every behavioral change lands once, upstream, tagged. The merge-queue ruleset template in tanh-tooling is applied to both repos. End state: **anira and tanh-lib "just" use tanh-ci + tanh-tooling; no duplicated CI code.** Web rides along (decided 2026-08-31): the wasm build goes through a shared `cmake-build-wasm` composite carrying the tanh-lab emsdk pin (a composite, not a reusable workflow, so anira's npm package build can follow in the same job over the emitted wasm); tanh-lib gains a `wasm-release` preset and a build-only wasm leg on the same action in its convergence PR — today tanh-lib is compiled under emscripten only *inside* anira's wasm build, so wasm breakage surfaces downstream instead of upstream.

**Step 9 — test-suite audit, coverage, and the badge** *(anira first, tanh-lib after)*
Three parts: (a) **audit the suite** — do all 307 tests carry weight? The 111 heavy model-inference tests are parameter sweeps (`InferenceTest` × backend × config); measure per-test value (unique failures caught, runtime cost) and prune or merge redundant parameterizations. (b) **coverage measurement** — an instrumented Linux leg (clang source-based coverage, `-fprofile-instr-generate`/`llvm-cov`) running the full suite and emitting a percentage. *Decided + implemented 2026-08-31*: it runs on **push to main** (a new build, so it does not violate the "main re-runs nothing" rule; the badge then reflects main exactly), via the `ci-tests-coverage` preset — Release stays, since clang's source-based coverage is accurate under optimization and the heavy suites keep their speed. Not queue-gated: a dip is information, never a merge blocker. (c) **the number on the README** — *decided 2026-08-31 after surveying the peer field*: **Codecov** (used by RTNeural, chowdsp_utils, Catch2, quill; the gist/shields approach is absent among comparable C++ libraries), uploaded tokenless via the OIDC flow with the action pinned by SHA (the 2021 uploader incident is why), badge at the top of the README. The per-file drill-down doubles as the audit's baseline metric. Remaining: (a), plus optionally PR diff-coverage comments later (would need uploads from PR tiers, which only run the fast suites — parked).
...
> **Recorded post-hoc (2026-08-31), user-directed in-session**: the tanh-tooling `hooks` install family (diff-based clang-tidy pre-push gate) and the `_anira_download` retry helper (PR #135) were both requested by Valentin during execution; neither traces to a roadmap step, which is why they are recorded here. The 2026-08-31 audit fixes: npm publish moved into on_tag behind the test gates (was an ungated tag-push workflow duplicating the emsdk setup), `--no-tests=error` on the benchmark gate, a fail-on-empty-glob guard on the Rosetta step (ci-actions v0.3.3), the step-4 `/Z7` invariant landed late, tanh-lib gained its promised warm_caches, `on_tag` got scoped permissions, and the secret-adjacent third-party actions are SHA-pinned.
>
> **Queue semantics (measured 2026-08-31)**: required status checks gate BOTH ends — they must have reported on the PR head before `enqueuePullRequest` is accepted ("expected" refusal otherwise, measured with a queue-only canary), and they are enforced again on the merge group (#139 landed only after its group's `install result` passed). Non-required group failures do NOT block a merge. Therefore every queue-only workflow carries a **PR-head stub**: a `pull_request` trigger under which the real jobs skip and only the `<name> result` job reports instant success (~5 s each) — which lets all results be required, making the queue's full sweep strictly enforcing. The one exception is `clang_tidy result`: its PR runs are path-filtered (diff-mode early warning), so it stays un-required; its queue run is visible but advisory. A queued PR's head branch is push-locked; dequeue via GraphQL `dequeuePullRequest` (select `clientMutationId`).

---

## 7. Next phase (opened 2026-08-31 evening, after the audit)

Decided order: 1 → 3 → 4 → 5 now, sanitizers (2) awaiting go.

1. **Codecov completeness** *(done in this phase)*: the first report had 21 files at 71.6% — `llvm-cov` reports only files compiled into the uploaded binaries, so the shared Linux leg silently omitted `ExecuTorchProcessor` (static-only), `TFLiteProcessor` (LiteRT chosen), the wasm wrappers and the benchmark fixture, overstating the number. Fix: a second **static-executorch-tflite** coverage leg (Codecov merges uploads), and `codecov.yml` excludes the wasm-only and benchmark sources *explicitly* so the percentage means what it says.
2. **Extended sanitizers** *(parked, needs go)*: ASan/UBSan/TSan/LSan wiring mirroring `ANIRA_WITH_RTSAN`, noengines scope (Q6), then the findings soak.
3. **Cache effectiveness** *(done in this phase)*: `cmake-build` (ci-actions v0.3.4) reports its sccache hit rate into every job's step summary and notes model-fixture cache misses — regressions like the months-long silent v1-protocol failure become glanceable. The macOS universal legs' by-design 0% (multi-`-arch` refused without `SCCACHE_CACHE_MULTIARCH`, flagged "may be unsound") gets an opt-in path: the `multiarch` matrix key, enabled on the three universal rows and validated by a full-tier dispatch (the Rosetta pass re-runs the whole suite on the second slice, which is the soundness check that matters here).
4. **Test-suite audit (step 9a)** *(running)*: multi-agent audit over the per-test timings of a full-tier leg (236 s serial across 273 timed tests) and the test sources; prune/merge proposals survive only adversarial verification. Output lands as a separate reviewed PR — test deletions are never bundled.
5. **Audit leftovers** *(done in this phase)*: sccache env blocks dropped from the five workflows whose only sccache consumer is `cmake-build` (which now exports the env itself; mobile and on_tag keep theirs for their raw-cmake jobs), `on_tag`'s hand-maintained `builddir` matrix column replaced by the shared `preset-binary-dir` resolver, `setup-cpp-build-tools` falls back to `runner.os` when a job has no `matrix.name`. Still parked by design: `DISCOVERY_MODE PRE_TEST` waits for a third macOS discovery flake.
