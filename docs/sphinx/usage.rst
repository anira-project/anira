...
- ``LogDrain::Thread`` (the default natively): a low-priority thread owned by the context — started with the first :cpp:class:`anira::InferenceHandler`, stopped and joined when the last one is destroyed (and by :cpp:func:`anira::Context::shutdown`), exactly like the inference thread pool. Nothing of it survives the last handler, so a plugin host may unload the library right after.
