...
**Solutions**:
    ...
    3. Restrict your plugin's exports to its entry points — e.g. on macOS ``-Wl,-exported_symbols_list`` with only ``_bundleEntry``/``_bundleExit``/``_GetPluginFactory`` for a VST3, on Linux a version script (``-Wl,--version-script``) with the same names under ``global:`` and ``local: *;`` (for a CLAP plugin the only name is ``clap_entry``; ``examples/clap-audio-plugin/cmake/clap.map`` and ``clap.exports`` are the two files, wired up in its ``CMakeLists.txt`` with ``LINK_DEPENDS``). This also covers any other statically linked dependency and is *required* on macOS when your plugin links a static anira with the ExecuTorch backend: its desktop archives are force-loaded, and ld64 has no hidden variant of ``-force_load``, so nothing short of an export list keeps ``executorch::``/``xnn_*`` out of the plugin's export table. Verify with ``nm -gU your_plugin`` (macOS) or ``nm -D --defined-only your_plugin.so`` (Linux): no ``Ort``/backend symbols should appear. anira runs the same check on its own binaries in CTest (``anira_exports``, ``cmake/check-exports.cmake``).
