...
``anira_pipeline_add_stage``; in C++ it is a subclass of :cpp:class:`anira::Stage` handed to
an :cpp:class:`anira::Pipeline` as :cpp:class:`anira::stage::Custom`. Both run on the 3.x
...
factor, two phases, one object. ``phases()`` names both, so the descriptor's ``pre_process``
and ``post_process`` slots are filled and the two hooks stay ``NULL``:
...
.. code-block:: cpp
    ...
    public:
        ...
        anira_status post_process(anira::StageContext& ctx) noexcept override {
            ...
            if (st != ANIRA_OK) { return st; }
            return anira::Stage::post_process(ctx);                    // the default push, every Streamed output
        }
...
has to be real-time itself: preallocated plans, no allocation in the call. The stage keeps its
window in a per-entry scratch sized in ``prepare``:
...
overlap-add keeps from one frame to the next is the stage's own business, in a scratch of its
own; the shape of the stage is the point here: pop the hop once, push the hop once, transform
in between.
...
.. code-block:: c
    ...
    typedef struct spectral { float* windows; } spectral;

    static anira_status ANIRA_CALL spectral_prepare(anira_handler* handler,
                                                    const anira_plan_report* report,
                                                    void* user_data) {
        spectral* s = (spectral*)user_data;
        (void)report;
        s->windows = (float*)calloc((size_t)anira_handler_num_entries(handler) * WINDOW, sizeof(float));
        return s->windows != NULL ? ANIRA_OK : ANIRA_ERROR_OUT_OF_MEMORY;
    }
    ...
    static void ANIRA_CALL spectral_release(void* user_data) {
        free(((spectral*)user_data)->windows);
    }

and its descriptor, filled once at setup:
...
chunk see the same ``entry``. ``anira_handler_num_entries`` is fixed by ``anira_handler_prepare``
for the life of the session and may change with the next prepare, which is why the scratch is
sized in the stage's ``prepare`` and not in its constructor. A ``pre_process`` that pops
nothing but the hop, as here, moves exactly what the hop rule expects.
...
a ``std::shared_ptr``; the pipeline's carrier shares the pointer, so the object lives at least
until the last pipeline or handler that carries it is destroyed, whatever you do with your own
pointer, and ``Stage::release()`` runs once at that moment:
