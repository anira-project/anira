...
A custom engine is benchmarked like a built-in one: it is created under its id and added to the pipeline (``anira_custom_engine_create`` and ``anira_pipeline_add_engine``, or an :cpp:class:`anira::Engine` through ``Pipeline::register_engine``; :doc:`custom_engines`), named by a model entry, and its plan is selected; the 2.x fixture of this pre-release, whose handler takes a :cpp:class:`anira::BackendBase`, selects that backend instead:
