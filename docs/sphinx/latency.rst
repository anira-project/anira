...
.. note::
    The Hard contract's geometry may be fractional (``block_min`` and ``block_max`` are doubles, e.g. ``contract.hard_geometry(samples_per_block / 2048.0, samples_per_block / 2048.0, sample_rate / 2048.0)`` for a model whose anchor stream runs at one sample per 2048 host samples; the rate is in anchor samples per second as well). The host and that stream then exchange samples at a non-integer ratio, and anira assumes the worst case: after its :math:`k`-th call the host has moved (pushed, or popped) :math:`\lfloor k b \rfloor` samples of every stream whose block is :math:`b`, so one call carries :math:`\lfloor b \rfloor` or :math:`\lceil b \rceil` of them. For example, with a block of 0.25 samples of a control-rate stream, the handler receives one sample every four host calls, and the latency is calculated as if the sample is delivered during the fourth call. If your system always sends the sample at the first call, a lower latency is possible; declare it with ``anira_contract_hard_set_latency`` (the ``latencies`` of ``anira::Hard``). :math:`B` is the geometry's ``block_max``, and :math:`\rho` the rational :cpp:func:`anira::LatencyCalculator::rationalize` recovers from it (a relative tolerance of :math:`10^{-6}` and a denominator of at most :math:`2^{20}`: 1/3 and 1/4 exactly). Nothing refuses a call that carries more; the rings are not sized for it, and a pop the stream cannot cover is a miss (``ANIRA_MISSED``).
