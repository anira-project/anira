...
anira_status anira_pipeline_register_engine(anira_pipeline*, const char* engine_id, const anira_backend_desc*, anira_error* err);   /* a custom engine of the inference stage: one more implementation its candidates resolve to, not a stage, no place in the chain order, legal before or after anira_pipeline_add_inference; engine_id reverse-URI; twice in one pipeline is ANIRA_ERROR_CONFIG; carrier */
...
namespace stage { struct Inference { Inference(const ModelConfig&, std::initializer_list<BackendId> = {}); Inference(std::initializer_list<std::reference_wrapper<const ModelConfig>>, std::initializer_list<BackendId>);
                                     Inference& engine(std::string_view engine_id, std::shared_ptr<BackendImpl>); };   // a custom engine is part of the inference stage: anira_pipeline_register_engine beside anira_pipeline_add_inference
