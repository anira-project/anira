...
typedef union anira_memory_handle {                        /* 24 bytes; every arm typeless; vendor typedefs replaced by their wire width */
    ...
    struct { ANIRA_PTR(void, buffer); uint64_t offset; }                      wgpu;      /* WGPUBuffer; same process, the core's Dawn device */
...
Rules: one tensor per submit; multi-buffering is a producer-side pattern over tokens (rotate descriptors, reuse a slot when its `input_released` token -- `anira_handler_ticket_input_released` -- signals) -- and it stays on the producer's side of the edge: an engine whose graph is captured replays the buffers it was captured with (section 7, completion), so the tensors bound to the engine are fixed and the edge moves each rotating slot into them. `anira_tensor_init_gl_buffer` requires the GL thread policy from `anira_context_config` (`anira_gl_desc.threads`, an `anira_gl_threads`). `anira_tensor_init_dmabuf` builds a Tensor from exported *buffer* memory (Vulkan `VK_KHR_external_memory_fd`, GBM linear bo, dma-heap) and is typed by the descriptor like every other arm. Images of any API (GL textures, `VkImage`, `WGPUTexture`, `MTLTexture`) are not Tensors: they are `Frame` containers and enter via a `FrameToTensor` stage, which is a zero-copy import plus one shader pass or an explicit device copy; the plan reports which.
...
/* the same pair on anira_tensor_spec, anira_model_config, anira_context_config, anira_contract, anira_job_options */
...
spec.ext(anira::ext::Quant{ ... });                                 // TensorSpec::ext, ModelConfig::ext, ContextConfig::ext, JobOptions::ext;
...
`anira_capabilities_ext_kinds` on `anira_context_capabilities(m)` reports the registered kinds beside the probed edges and the enabled backends, and `anira_registered_ext_kinds` reports them without a context, so a deployment can ask whether this anira understands `npu` before loading a model that needs it.
...
anira_status anira_tensor_spec_set_window    (anira_tensor_spec*, int64_t window_min, int64_t window_max, int64_t overlap);
        /* window_min: model's smallest legal Time extent; window_max: largest, ANIRA_UNBOUNDED = no upper limit;
           overlap: elements kept from the previous window; the hop, consumed per inference, is window_used - overlap;
...
public:
    ...
    // ANIRA_ROLE_STREAMED only, in elements along the Time axis:
    TensorSpec& window(int64_t window_min, int64_t window_max, int64_t overlap);   // unwritten: 0, 0, 0
...
## 4. ContextConfig (context and process resources)

`ContextConfig` is an opaque handle, `anira_context_config`, mutated by scalar setters: threads and wait strategy, the log block, one Tier-2 descriptor per device API, extensions. The handle's layout never enters the ABI; the device descriptors are `struct_size`-first records handed once to a setter and read only inside `min(struct_size, sizeof(lib's))`, so a block grows at the tail inside ABI major 3, and JS -- which never writes a struct field by hand -- builds a context config with one call per field. A NULL descriptor is an absent block: the domain is unavailable and its edges are pruned. Presence is the user's declaration; the Context probes what is declared and nothing else.

Two actors share this section, and ownership follows them. The *core* is the one immortal state of a copy of anira: it owns the process resources -- the default thread pool, the log queue and its drain, the engine environments, the Dawn instance and the one Dawn device, and the steady clock and shutdown family of `abi/core.h`. A *context* is the user's handle over that core, created first and possibly several times per process: its config declares what the user brings (device blocks, a sink, a thread count, the log settings), and the handle is where the probe runs and where capabilities and reports are read. What arrives in a device block stays the user's, borrowed for the context's lifetime and reconciled into the core the way sessions are; what the core creates itself (the Dawn instance, an engine environment) is the core's and outlives any one context.
...
anira_status anira_context_config_create(anira_context_config** out, anira_error* err);
anira_status anira_context_config_set_threads(anira_context_config*, uint32_t num_threads, anira_wait_strategy);   /* defaults: ANIRA_THREADS_AUTO, SPIN_BACKOFF */
anira_status anira_context_config_set_log_level(anira_context_config*, anira_log_level);                     /* default WARNING; most verbose request wins */
anira_status anira_context_config_set_log_drain(anira_context_config*, anira_log_drain, uint32_t interval_ms);
anira_status anira_context_config_set_log_queue_capacity(anira_context_config*, uint32_t capacity);          /* clamped [64, 65536]; fixed for the life of the core */
anira_status anira_context_config_set_log_flags(anira_context_config*, uint32_t flags);
anira_status anira_context_config_set_log_sink(anira_context_config*, anira_log_fn, void* user_data);        /* ignored on Wasm: anira_em_set_log_hook is the sink there */
anira_status anira_context_config_set_log(anira_context_config*, const anira_log_desc*);                     /* C one-shot convenience = the five above */
anira_status anira_context_config_set_cuda  (anira_context_config*, const anira_cuda_desc*);     /* NULL = domain unavailable, edges pruned */
anira_status anira_context_config_set_gl    (anira_context_config*, const anira_gl_desc*);       /* presence is the user's declaration, no implicit probing */
anira_status anira_context_config_set_vulkan(anira_context_config*, const anira_vulkan_desc*);
anira_status anira_context_config_set_metal (anira_context_config*, const anira_metal_desc*);
anira_status anira_context_config_set_d3d12 (anira_context_config*, const anira_d3d12_desc*);
anira_status anira_context_config_set_webgpu(anira_context_config*, const anira_webgpu_desc*);  /* native Dawn; Emscripten: ANIRA_ERROR_NOT_SUPPORTED in 3.0 */
anira_status anira_context_config_set_ext(anira_context_config*, const anira_ext_header*, anira_error* err);           /* section 1b */
anira_status anira_context_config_set_ext_json(anira_context_config*, const char* kind, const char* utf8, size_t len, anira_error* err);
anira_status anira_context_config_from_json(const char* utf8, size_t len, anira_context_config** out, anira_error* err);   /* section 8; JSON device blocks = OWNED */
anira_status anira_context_config_to_json(const anira_context_config*, char* buf, size_t cap, size_t* out_len);
void         anira_context_config_destroy(anira_context_config*);

/* abi/context.h -- the handle over the core, its capabilities, the probe */
typedef struct anira_context anira_context;  typedef struct anira_capabilities anira_capabilities;
...
anira_status anira_context_create(const anira_context_config*, anira_context** out, anira_error* err);   /* [main-thread]; probes, validates borrowed devices, Dawn assertion, registers the sink */
void         anira_context_destroy(anira_context*);            /* [main-thread & !loader-lock]; drops the user's reference, unregisters the sink, joins nothing */
anira_status anira_context_probe(anira_context*, anira_bool force, anira_error* err);   /* [main-thread]; re-runs the rungs, refreshes capabilities */
const anira_capabilities* anira_context_capabilities(const anira_context*);              /* [thread-safe]; context-owned */
...
anira_status anira_enabled_backends(uint32_t element_size, uint32_t* count, anira_backend_id* out);   /* [thread-safe]; no context needed: what this build compiled in */
uint64_t     anira_context_byte_image_bytes(const anira_context*, uint64_t num_elements, anira_dtype);   /* [thread-safe]; the edge's encoding, section 7 */

/* abi/core.h -- the core of this copy: its steady clock and the shutdown family */
...
anira_status anira_shutdown(void);                            /* [main-thread & !loader-lock]; idempotent, never creates the core;
                                                                 effective only when no context handle and no handler exists in this copy,
...
The context handle. `anira_context` is a refcounted handle over the immortal core -- one core per copy of anira (a shared `libanira.so.3` has one, every static embedding has its own), the object that is `Core` today. `anira_context_create` `[main-thread]` reconciles the config into the core, validates borrowed devices, runs the Dawn version assertion and the probe below, registers the log sink and returns the handle; `anira_handler_create` adds a reference, so the handle's memory (reconciled config, device blocks, capabilities) outlives the user's `anira_context_destroy`, which drops the user's reference, unregisters the sink, invalidates the handle for the caller regardless of the count, and joins nothing. Thread pool and inference queue are core-owned and exist exactly while any handler in this copy exists: the first context sizes the pool; later contexts reconcile per field as today -- wait strategy first wins, log level most verbose wins, the pool only shrinks and never to zero (`0` = no preference while handlers exist), drain mode, queue capacity and drain interval first win with a warning on mismatch -- and once the last handler is gone the next context's config takes effect whole. `anira_num_inference_threads` (`abi/thread.h`, section 6) reports the size of that default pool and, like `anira_enabled_backends`, takes no context. One configuration type: the core reads `anira_context_config` itself. `Core::register_context` and `Core::create_session` take the C struct, `sanitize_config` resolves `ANIRA_THREADS_AUTO` and the WebAssembly coercions on it, the reconciliation compares its scalars, and the C enums `anira_wait_strategy`, `anira_log_level` and `anira_log_drain` reach `InferenceThread`, the log queue and the drain unconverted; the handle keeps its copy (its log flags are what destroy releases; a handler passes the copy through to its session unchanged), the core its reconciled one. The consumed-or-fail walk over the context's extension bag runs once, at `anira_context_create`, before the sink is registered. A handler is a user of the core beside the contexts and the sessions (`Core::register_handler`), which is what keeps `anira_shutdown` refused while one lives. The 2.x `InferenceManager`, still public until the cut-over, maps its `CoreConfig` onto the C struct at construction (`anira::capi::make_context_config`) and is the only remaining consumer of the 2.x enums; the mapping leaves with the 2.x classes. Two context handles in one copy are two views of one core with two log sinks, sharing the default pool rather than owning one each; `anira_shutdown` (section 6) is refused while any of them or any handler exists. `CoreConfig::m_anira_version` and `m_enabled_backends` have no successor fields: what the build compiled in is `anira_enabled_backends`, a function that needs no context; what is usable *here* -- a compiled-in backend whose driver is absent is enabled but not usable -- is `anira_capabilities_backends` on the probed `anira_capabilities`; the cross-session version compare is dropped, because inside one copy every session shares one header and across two copies nothing can see the other.

The same object in C++ is a builder over the handle, and `anira::Context` owns one `anira_context*`:

```cpp
class ContextConfig { static ContextConfig from_json(std::string_view); static ContextConfig from_file(const std::filesystem::path&);
    ContextConfig& threads(uint32_t num_threads, anira_wait_strategy = ANIRA_WAIT_SPIN_BACKOFF);
    ContextConfig& log_level(anira_log_level); ContextConfig& log_sink(LogSink); ContextConfig& log(const anira_log_desc&);
    ContextConfig& cuda(const anira_cuda_desc&); ContextConfig& gl(const anira_gl_desc&); ContextConfig& vulkan(const anira_vulkan_desc&);
    ContextConfig& metal(const anira_metal_desc&); ContextConfig& d3d12(const anira_d3d12_desc&); ContextConfig& webgpu(const anira_webgpu_desc&); };
class Context { explicit Context(const ContextConfig&); const Capabilities& capabilities() const; void probe(bool force = false);
                size_t drain_log(); anira_context* native() const; };

anira::ContextConfig mc;
...
anira::Context context{mc};                                                                               // anira::Error on ANIRA_FAILED
```

`anira_context_config_set_ext`/`set_ext_json` carry the section 1b payloads (`ext::Npu` and `ext::ArtifactCache` arrive with their adapters); the device blocks are setters, not extensions, because the Context's probe reads them, not an adapter. `anira_context_config_from_json` reads the `context.json` of section 8, whose device blocks imply `ANIRA_OWNERSHIP_OWNED`; borrowed handles are code-only and are patched onto the loaded handle by the setters afterwards.

WebGPU ownership: one WebGPU implementation per process, and anira owns it. The core links its own Dawn (one per copy of anira) (`libwebgpu_dawn.so`, monolithic shared build from a pinned Dawn revision) and creates or borrows the instance, device and queue on it -- `WGPUInstance`/`WGPUDevice`/`WGPUQueue`, carried as `void*` in `anira_webgpu_desc` because the header names no vendor type. Engines never bring their own Dawn; they receive anira's through a proc table: a `WGPUDevice` is a C++ object of one build, not an ABI-stable handle, and `DawnProcTable` is a struct whose layout is fixed by the Dawn revision, so engine and Context must be built against the same Dawn source tree. Per engine:
...
Cost and caching. Functional probes are milliseconds each and there are dozens, and a plugin constructs its Context inside `prepareToPlay`. Results are therefore cached on the box, keyed by `(anira version, driver versions, device UUIDs, enabled blocks)`, next to where an artifact cache would live: a matching key loads the registry in microseconds, a mismatch re-probes. `anira_context_probe(m, force, err)` re-runs on demand and refreshes the `anira_capabilities` object that `anira_context_capabilities` returns, and a benchmark run always forces, since a row cached under last month's driver measures nothing.

Thread and device contracts. GL rungs need the borrowed context current and run on the thread that calls `anira_context_create` under `ANIRA_GL_CALLER_THREAD`, as every GL touch does. Functional probes submit once to the Vulkan, Dawn and CUDA queues at construction; on an `ANIRA_OWNERSHIP_BORROWED` device that is a submission the application did not make, which is part of what borrowing means and is stated here so that it is not a surprise.

How a thread waits for a GPU is a context-level decision, not a detail: `anira_wait_strategy` governs GPU completion waits as much as queue waits. A thread that blocks in a fence wait for the length of an inference leaves its core idle, and the CPU frequency governor clocks it down; the *next* thing that runs there -- the pre-processing of the following frame, the host callback -- then executes slow. Measured on the M1 under `schedutil`: identical producer code took 46 us when it followed a CPU inference and 208 us when it followed a 10 ms GPU fence wait, edges 113 us versus 28 us, and even the engine's own submission overhead grew by ~0.7 ms; pinning the governor to `performance` collapsed every column onto the busy-core value. Consequences: `ANIRA_WAIT_SPIN_BACKOFF` is the correct default for deadline-carrying contracts (Hard always, Async with a deadline), a blocking wait is for offline postures, and the plan report names the wait strategy used per edge (`anira_plan_slot.wait_strategy`). Benchmarks record the governor (see section 7) -- otherwise the two regimes differ by 3-5x on everything except the inference itself and the numbers are not comparable.
...
One `ModelConfig` describes one logical model. A variant set references several configs inside one handler; running models in sequence is several handlers on one Context, composed through tickets (section 7, Multi-model support).
...
The handler surface is four C headers -- `abi/context.h`, `abi/thread.h`, `abi/handler.h` (M2) and `abi/ticket.h` (M3) -- plus the job-options setters of `abi/config.h`; `anira.hpp` is the C++ spelling over them and adds nothing the C header cannot do. `abi/context.h` is section 4's block; the handler surface starts here, at the inference threads a host drives itself, which exist only when the context was configured with `num_threads == 0` (section 4) and which are the Wasm build's only inference threads (section 6b). Export macro, error model, ownership, enumeration convention and the thread-tag vocabulary are stated once in section 6a and assumed here; in the blocks below `[tag]` is the thread tag, `[cs]` = `[callback-safe]`, `NB` = `ANIRA_NONBLOCKING` after the declarator, and the `ANIRA_API`/`ANIRA_CALL` pair is elided for readability and present in the real header.
...
anira_status anira_inference_thread_create(anira_context*, anira_inference_thread** out, anira_error* err);   /* [main-thread]; bound to the core's queue, outlives the context; Wasm: main instance only */
...
void         anira_inference_thread_start(anira_inference_thread*);       /* [main-thread]; native: spawns thl::core::Thread at the context's inference-thread scheduling class (RealTime by default) */
...
anira_status anira_pipeline_add_inference(anira_pipeline*, const anira_model_config* const* variants, uint32_t num_variants,
                                          const anira_backend_id* candidates, uint32_t num_candidates, anira_error* err);   /* NULL/0 candidates = every engine the variants name, on ANIRA_PROVIDER_CPU */
anira_status anira_custom_engine_create(const anira_engine_desc*, anira_custom_engine** out, anira_error* err);   /* the engine object: the descriptor in a refcounted carrier; the identity the pool shares prepared models by */
void anira_custom_engine_destroy(anira_custom_engine*);   /* drops the handle's reference; pipelines, handlers and prepared models keep theirs */
anira_status anira_pipeline_add_engine(anira_pipeline*, const char* engine_id, const anira_custom_engine*, anira_error* err);   /* a custom engine of the inference stage: one more implementation its candidates resolve to, not a stage, no place in the chain order, legal before or after anira_pipeline_add_inference; engine_id reverse-URI and the pipeline's own (one engine may sit in several pipelines under several ids); an id twice in one pipeline is ANIRA_ERROR_INVALID_STATE */
void         anira_pipeline_destroy(anira_pipeline*);
anira_status anira_handler_create(anira_context*, const anira_pipeline*, anira_handler** out, anira_error* err);   /* [main-thread]; loads every candidate's model whose prepare != NULL; add-refs the context; copies everything */
...
/* ---- _wait twins: v2's blocking_ratio / set_non_realtime waits; [any-thread, blocking], NOT NB; timeout_ms >= 0 explicit, ANIRA_WAIT_CONTRACT, ANIRA_WAIT_FOREVER;
        semaphore wait when wait_ratio > 0, 1 ms atomic poll otherwise (Core.cpp:948-956); 0 / ANIRA_ERROR_INVALID_STATE at once without an active inference thread; Wasm: spins ---- */
...
The same surface in `anira.hpp` (header-only, C++20, `namespace anira`, not ABI-stable; nothing in it is exported from libanira). `Hard`, `Async` and `Contract` are section 3's aggregates, `Context` and `ContextConfig` are section 4's; what follows is the handler half and the types a stage or a custom backend subclasses:
...
namespace stage { struct Inference { Inference(const ModelConfig&, std::initializer_list<BackendId> = {}); Inference(std::initializer_list<std::reference_wrapper<const ModelConfig>>, std::initializer_list<BackendId>);
                                     Inference& engine(std::string_view engine_id, std::shared_ptr<BackendImpl>); };   // a custom engine is part of the inference stage: anira_pipeline_add_engine beside anira_pipeline_add_inference
...
class Pipeline { Pipeline(std::initializer_list<std::variant<stage::Inference, stage::Custom>>); Pipeline& inference(const ModelConfig&, std::initializer_list<BackendId> = {});
                 Pipeline& register_engine(std::string_view engine_id, std::shared_ptr<BackendImpl>); ... };   // anira_pipeline_register_engine, as the TypeScript registerEngine; no stage::CustomBackend: an engine is no stage
...
class InferenceHandler {
    InferenceHandler(Context&, const Pipeline&);
    ...
    void set_model(uint32_t variant) noexcept ANIRA_NONBLOCKING;  void set_backend(Engine, Provider = ANIRA_PROVIDER_CPU) noexcept ANIRA_NONBLOCKING;   // conveniences over set_plan, O(1) through the (variant, engine, provider) -> plan table built at prepare
    // Async entries
...
class InferenceThread { explicit InferenceThread(Context&); void start(); void stop(); bool execute(); void run_loop();
...
| `abi/tensor.h`, `abi/context.h`, `abi/thread.h`, `abi/stage.h`, `abi/handler.h` | RT PODs; context, capabilities, lifecycle; user-driven inference threads; rings, stage and backend descriptors; pipeline, handler, Hard entries and their `_wait` twins | M2 |
| `abi/ticket.h` | Async entries | M3 |
...
- Context: a refcounted handle over the immortal core (`include/anira/scheduler/Core.h:79-81`, one core per copy of anira). `anira_handler_create` adds a reference so the handle's memory outlives the user's `anira_context_destroy`; `anira_context_destroy` drops the user's reference, invalidates the handle for the caller regardless of the count, unregisters its log sink and waits for that sink's in-flight calls, and joins nothing. Thread pool and inference queue are core-owned: the pool exists exactly while any handler in this copy exists; user-driven `anira_inference_thread` objects bind to the core's queue and outlive contexts. `num_threads == 0` keeps meaning "bring your own threads" (`CoreConfig.h:221-231`); `ANIRA_THREADS_AUTO = 0xffffffff` is the library default.
- Shutdown, C-visible: `anira_shutdown(void)` is `Core::shutdown()` (`Core.cpp:682-704`, idempotent, never creates the core), effective only when no context handle and no handler exist in this copy; otherwise it does nothing and returns `ANIRA_ERROR_INVALID_STATE`, so a client of a shared `libanira.so.3` cannot silence another client's sessions. It is meant for static embeddings and is called from `clap_deinit`/`ExitDll` as `examples/clap-audio-plugin/anira-clap-demo-pluginentry.cpp:42-49` does today. `anira_release_core_if_idle()` never blocks (on Wasm it also requires zero loop-active threads) and `anira_has_core()` stay. The unload rule: the ELF/Mach-O destructor hook (`Core.cpp:131-146`) stays as a backstop for hosts that skipped `anira_handler_destroy`, but v3 runs host code (`anira_backend_desc.process`, `before/after_inference`, `on_complete`, tensor `release`) on pool threads, so a host that installs custom stages or backends must destroy its handlers before its DSO is unloaded -- the hook joins under the loader lock and host code that binds symbols there deadlocks. Windows keeps the non-joining `UnloadGuard`; `anira_context_destroy` and `anira_shutdown` are `[main-thread & !loader-lock]`. User-managed `anira_inference_thread` objects must be stopped before unload; on Wasm `anira_inference_thread_destroy` additionally requires `has_exited`.
...
| log sink `anira_log_fn` | any thread that logs: the caller of every `[main-thread]` entry (sync records), the caller of `anira_context_destroy`/`anira_shutdown` (final flush), `[drain-thread]` for RT records; never the driver thread; may run with anira's lifecycle lock held | not RT, may allocate, must not call anira | `anira_log_desc` |
...
`[callback-safe]` is an additive property: from inside any callback a host may call exactly the entries so tagged and nothing else -- `anira_tensor_init_*`, `anira_tensor_data_f32`, `anira_tensor_data` (a converting stage needs the typed read from inside a callback), `anira_tensor_num_elements`, `anira_tensor_extent`, `anira_sizeof`, `anira_ring_*`, `anira_stage_default_pre_process/post_process`, `anira_log_rt`, `anira_now_ms`/`anira_now_ns`, `anira_handler_rt_error`, `anira_handler_ticket_status`, `ticket_input_released`, `ticket_output_ready`, `ticket_error`, `ticket_user_data`, `ticket_release`; a stage's `prepare` may additionally use the handler getters and `set_input`/`get_output`. The reason is a real lock: `anira_backend_desc.prepare` runs inside `create_session` under `m_lifecycle_mutex` (`Core.cpp:530-535`) and sync log records are dispatched while that non-recursive mutex is held, so a `prepare` or log callback that calls `anira_context_create`, `anira_handler_create/destroy` or `anira_shutdown` deadlocks. `ticket_release` from `on_complete` is legal (the slot recycles after the callback returns); `submit` is not. On Wasm JS supplies no function pointers; `anira_em.h` ships trampolines (`anira_em_stage_desc_init`, `anira_em_backend_desc_init`, `anira_em_job_options_set_on_complete_js`, `anira_em_set_log_hook`) that fill the same slots with C functions compiled into anira (section 6b).
...
```c
typedef enum anira_engine   { ANIRA_ENGINE_NONE = 0, ANIRA_ENGINE_ONNXRUNTIME = 1, ANIRA_ENGINE_EXECUTORCH = 2, ANIRA_ENGINE_LITERT = 3,
    ANIRA_ENGINE_LIBTORCH = 4, ANIRA_ENGINE_TFLITE = 5,
    /* 6 .. 0x7ffffffd reserved for later built-in engines, appended after TFLITE */
    ANIRA_ENGINE_CUSTOM = 0x7ffffffe /* a custom engine, named by its id */,
    ANIRA_ENGINE_FORCE32 = 0x7fffffff } anira_engine;
typedef enum anira_provider { ANIRA_PROVIDER_NONE = 0 /* no provider */, ANIRA_PROVIDER_CPU = 1 /* the engine's plain CPU path */,
    ANIRA_PROVIDER_CUDA = 2, ANIRA_PROVIDER_WEBGPU = 3, ANIRA_PROVIDER_VULKAN = 4, ANIRA_PROVIDER_DIRECTML = 5,
    ANIRA_PROVIDER_COREML = 6, ANIRA_PROVIDER_XNNPACK = 7,
    /* 8 .. 0x7ffffffd reserved for later providers, appended after XNNPACK */
    ANIRA_PROVIDER_CUSTOM = 0x7ffffffe /* a provider named by its id */,
...
| `ANIRA_ENGINE_EXECUTORCH` | `2` | `EXECUTORCH` / `"executorch"` |
| `ANIRA_ENGINE_LITERT` | `3` | `LITERT` / `"litert"` |
| `ANIRA_ENGINE_LIBTORCH` | `4` | `LIBTORCH` / `"libtorch"` |
| `ANIRA_ENGINE_TFLITE` | `5` | `TFLITE` / `"tflite"` |
| `ANIRA_ENGINE_CUSTOM` | `0x7ffffffe` | `CUSTOM` / the engine's own id |
| `ANIRA_ENGINE_FORCE32` | `0x7fffffff` | -- |

| `anira_provider` | value | JSON suffix |
|---|---|---|
| `ANIRA_PROVIDER_NONE` | `0` | -- (no provider) |
| `ANIRA_PROVIDER_CPU` | `1` | -- (omitted) |
| `ANIRA_PROVIDER_CUDA` | `2` | `:cuda` |
| `ANIRA_PROVIDER_WEBGPU` | `3` | `:webgpu` |
| `ANIRA_PROVIDER_VULKAN` | `4` | `:vulkan` |
| `ANIRA_PROVIDER_DIRECTML` | `5` | `:directml` |
| `ANIRA_PROVIDER_COREML` | `6` | `:coreml` |
| `ANIRA_PROVIDER_XNNPACK` | `7` | `:xnnpack` |
| `ANIRA_PROVIDER_CUSTOM` | `0x7ffffffe` | the provider's own name |
| `ANIRA_PROVIDER_FORCE32` | `0x7fffffff` | -- |

A v2 `InferenceBackend` names an engine and never a provider, so v2 `ONNX` is `ANIRA_ENGINE_ONNXRUNTIME` with `ANIRA_PROVIDER_CPU`, `LIBTORCH` is `ANIRA_ENGINE_LIBTORCH` with `ANIRA_PROVIDER_CPU`, and so on through the five; v2 `CUSTOM` is a registered engine name, not a value.
...
- `[driver-thread]` -- the thread that drives a Hard contract: the host's real-time callback, whatever the medium -- an audio device callback, a video frame callback, a render loop. `ANIRA_NONBLOCKING`, not concurrent per handler: two driver threads may drive two handlers of one context, never one handler. "Not concurrent" holds per handler, not per context. The tag names the role, not the medium, which is why `pre_process` carries it under Hard and `[inference-thread]` under Async.
...
| `utils/Logger.h:4` and `scheduler/Core.h:4,542` include `<tanh/core/Logger.h>`; `utils/Logger.h:112` exports `rt_log_queue_slot()` returning `std::atomic<thl::Logger::rt::Queue*>&` | both headers private; logging crosses as `anira_log_desc` in and `anira_log_record` out (section 4); the slot stays constant-initialised per copy |
...
anira_status anira_em_pipeline_register_js_engine(anira_pipeline*, const char* engine_id, uint32_t js_key, anira_error* err);   /* == desc_init + anira_custom_engine_create + anira_pipeline_add_engine under the same name; engine_id is reverse-URI */
...
/* reserved for post-3.0: anira_em_context_config_set_js_webgpu(mc, js_key, err) -- the browser GPUDevice import, with the asynchronous backend contract below */
/* Module.anira = { version: number, stage(key, phase, a, b), backend(key, engineIdPtr, inPtr, nIn, outPtr, nOut, instance), log(recordPtr), complete(key, handlerPtr, ticket) }
   phase is an anira_stage_phase. PRE_PROCESS, POST_PROCESS, BEFORE_INFERENCE, AFTER_INFERENCE: a = ctxPtr, b = 0; PREPARE: a = handlerPtr, b = planReportPtr; RELEASE: a = b = 0.
   ANIRA_PHASE_INFERENCE never reaches stage(): the engine call of a JS engine is backend().
...
| `config/` | `TensorSpec`, `ModelConfig`, `ContextConfig`, `Contract` (`Hard`/`Async`), `JobOptions` -- builders, one C call per setter, `fromJson()`/`fromUrl()`, `destroy()` eagerly after handler creation |
| `core/` | `Context`, `Pipeline`, `InferenceHandler` with `process*` and `processWait*`, `PlanReport`, `Capabilities`, `Ticket` (`submit(inputs, opts?, deadlineMs?, jobUserData?)`, `userData` reading the pointer back as a number), `Tensor` (host: owns its heap block, freed after a terminal `status()`), `InferenceThread` |
...
const handler = context.createHandler(Pipeline.inference(cfg, ['org.anira.onnxruntime-web']).registerEngine('org.anira.onnxruntime-web', new ONNXRuntimeWebBackend(cfg)))
...
typedef enum anira_stage_phase { ANIRA_PHASE_PRE_PROCESS = 0, ANIRA_PHASE_POST_PROCESS = 1, ANIRA_PHASE_BEFORE_INFERENCE = 2,   /* abi/enums.h, pinned at M1 */
    ANIRA_PHASE_INFERENCE = 3,                 /* the engine call itself, between BEFORE and AFTER: no stage callback runs in it; the word a failure, a log record or a report row uses for the engine call */
    ANIRA_PHASE_AFTER_INFERENCE = 4, ANIRA_PHASE_PREPARE = 5, ANIRA_PHASE_RELEASE = 6, ANIRA_STAGE_PHASE_FORCE32 = 0x7fffffff } anira_stage_phase;
...
anira_backend_id candidates[] = {                                       /* engine and provider are independent axes; engine_id is NULL for a built-in engine */
    ...
    { sizeof(anira_backend_id), ANIRA_ENGINE_TFLITE,      ANIRA_PROVIDER_CPU, NULL },
};
...
Deferred past v1 (section 1a); it arrives with `abi/draft/frame.h` and `submit_frame`, additive under ABI major 3. When it ships, its kernels run where anira already has a device -- WGSL on the Context's Dawn, C on the host -- so the stage adds no domain; a SPIR-V or CUDA kernel arrives with an engine that reads that domain. A same-process image the user never exported stays theirs to convert.
...
### 8.2 Context file (`context.json`, lives on the box)
...
`vulkan.device` names a device index that `anira_vulkan_desc` has no slot for (open item 2 of section 12): the M1 loader keeps it on the handle, and the M4 context selects the physical device by it when it creates the device (a borrowed device carries its own).

`"num_threads": 0` means bring your own threads, as it does in the tree and as the Wasm build requires; the library default is the absent key (`ANIRA_THREADS_AUTO`, section 4). `"log"` is the block the tree already parses (`level`, `drain`, `queue_capacity`, `drain_interval_ms`, the four scalar setters of section 4); the bare `log_level` key is the version 2 spelling and is upgraded, not accepted, in a v3 document. The sink is a callback and is code-only (`anira_context_config_set_log_sink`; `anira_em_set_log_hook` on Wasm). Device blocks in JSON imply `ANIRA_OWNERSHIP_OWNED` (anira creates); `"d3d12"` and `"webgpu"` are spelled the same way and a block for a device this build cannot drive fails `anira_context_create` with `ANIRA_ERROR_NOT_SUPPORTED`, which is the Context-construction half of the validation rule above. `"npu"` and `"artifact_cache_dir"` return as extension keys with the adapters that consume them (section 1b); until then a file that carries them fails `anira_handler_prepare` by name. Borrowed handles are code-only, patched onto the loaded handle by the device setters, which replace the block they name:

```c
anira_context_config* mc = NULL;
anira_context_config_from_json(text, len, &mc, &err);
...
anira_context_config_set_gl(mc, &gl);
anira_context* m = NULL;
anira_context_create(mc, &m, &err);                    /* validates: indices exist, borrowed non-null */
anira_context_config_destroy(mc);                      /* the context copied it */
```

```cpp
auto mc = anira::JsonConfigLoader::context("context.json");   // == anira::ContextConfig::from_file
mc.gl(gl);                                                    // the same anira_gl_desc
anira::Context context(mc);
```
...
| `processing_spec.preprocess_input_size[i]` / `postprocess_output_size[i]` | `inputs[i].window` / `outputs[i].window` with `min == max` = the per-channel element count (elements / channels); `overlap` = that count minus the size (the receptive-field rule) | `0` = `"role": "static"` |
...
anira::Context context{anira::ContextConfig::from_file("context.json")};
anira::InferenceHandler handler{context, anira::Pipeline{ anira::stage::Inference(cfg) }};
...
anira_model_config* cfg; anira_context_config* mc; anira_context* m; anira_pipeline* pipe; anira_handler* h;
...
anira_context_config_create(&mc, &err);
anira_context_config_set_log_level(mc, ANIRA_LOG_WARNING);
anira_context_config_set_log_sink(mc, log_sink, host);                                       /* anira_log_fn: never the driver thread; must not call anira */
if (ANIRA_FAILED(anira_context_create(mc, &m, &err))) { host_log(err.message); return false; }
anira_pipeline_create(&pipe, &err);
const anira_model_config* variants[] = { cfg };
anira_backend_id candidates[] = {                                                            /* engine and provider are independent axes; engine_id is NULL for a built-in engine */
    { sizeof(anira_backend_id), ANIRA_ENGINE_ONNXRUNTIME, ANIRA_PROVIDER_CPU, NULL },
    { sizeof(anira_backend_id), ANIRA_ENGINE_LITERT,      ANIRA_PROVIDER_CPU, NULL },
};
...
if (ANIRA_FAILED(st)) { host_log(err.message); anira_context_destroy(m); return false; }
...
anira_context_destroy(m);                                                                    /* drops the reference, joins nothing */

/* clap_entry.deinit [main-thread & !loader-lock]: the host's last call before it unloads the DSO */
void clap_deinit(void) { anira_shutdown(); }                                                 /* idempotent; refuses (ANIRA_ERROR_INVALID_STATE, nothing happens) while any context or handler of this copy still lives */
```
...
const context = anira.createContext({ log: { level: Abi.LOG_WARNING, sink: r => console.log(r.group, r.message) } });   // _anira_context_config_set_log_level + _anira_em_set_log_hook + drain timer
const handler = context.createHandler(Pipeline.inference(cfg, ['org.anira.onnxruntime-web']).registerEngine('org.anira.onnxruntime-web', new ONNXRuntimeWebBackend(cfg)));   // _anira_em_pipeline_register_js_engine
cfg.destroy();
...
```cpp
auto mc = anira::ContextConfig::from_file("context.json");
...
anira::Context context{mc};
anira::InferenceHandler handler{context, pipe};                    // Inference(cfg, {{ANIRA_ENGINE_ONNXRUNTIME, ANIRA_PROVIDER_CUDA}})
...
anira::Context context{mc};
anira::InferenceHandler handler{context, anira::Pipeline{ anira::stage::Inference(palm_cfg, {{ANIRA_ENGINE_ONNXRUNTIME, ANIRA_PROVIDER_WEBGPU}}) }};
...
```cpp
anira::Context context{mc};                                          // one Dawn device, shared (native WebGPU; the browser GPUDevice is post-3.0)
anira::InferenceHandler palm{context, anira::Pipeline{ anira::stage::Inference(palm_cfg, {{ANIRA_ENGINE_ONNXRUNTIME, ANIRA_PROVIDER_WEBGPU}}) }};
anira::InferenceHandler lmk {context, anira::Pipeline{ anira::stage::Inference(lmk_cfg,  {{ANIRA_ENGINE_ONNXRUNTIME, ANIRA_PROVIDER_WEBGPU}}) }};
...
if (prev_rects.size() < num_hands) {                                 // the gate: application logic
    write_letterbox(palm_in, frame);                                 // the app's pass on the core's Dawn queue
...
anira::InferenceHandler handler{context, pipe};
handler.prepare(anira::Async{ .deadline = 33ms, .on_late = ANIRA_LATE_DROP });
...
- `anira_shutdown` is effective only when no context handle and no handler exist in this copy of anira; otherwise it does nothing and returns `ANIRA_ERROR_INVALID_STATE`, so one client of a shared `libanira.so.3` cannot silence another's sessions.
...
The v2 renames -- `InferenceConfig` -> `ModelConfig`, `CoreConfig` -> `ContextConfig`, `Core` -> `Context`; `TensorShapeList` + `ProcessingSpec` -> `TensorSpec` (shapes -> tagged axes, `preprocess_input_size` -> `window_min == window_max`, hop -> `overlap`); `prepare(HostConfig)` -> `prepare(Hard{...})` (the custom-latency overloads -> per-output `latency` on the spec); `max_inference_time` / `warm_up` / `blocking_ratio` -> the Hard fields `budget`, `warmup`, `wait_ratio`; `model_function` -> `ext::Entry{name}` (`anira_ext_entry`, kind `"entry"`) on the model entry, the one extension v3.0.0 ships -- are constructor-level shims in `namespace anira::v2`, every entity `[[deprecated]]`, bridging one minor cycle and removed one minor after 3.0:
...
| `CoreConfig(unsigned num_threads = default_num_threads(), WaitStrategy = SpinBackoff, LogLevel = default_log_level())`; `using Context = anira::Context` | a `ContextConfig`, with v2's defaults reproduced, not v3's: `default_num_threads()` is `hardware_concurrency() / 2` (minimum 1) natively and 0 on Emscripten, `default_log_level()` is `Info` in debug and `Error` in release (`CoreConfig.h:88-99,167-179`), where `anira_context_config` defaults to `ANIRA_THREADS_AUTO` and `ANIRA_LOG_WARNING` (section 4) |
...
Everything in sections 1b, 2, 3, 4 (the struct), 5, 6 (`edge_cost`) and 8 that is data, spelled as C handles with scalar setters: `anira_tensor_spec` with tagged axes, roles, window/context/latency; `anira_model_config` with models by index, the per-entry tensor record (`set_tensor_name`, `set_tensor_layout`), `set_model_bytes`, `set_anchor` by canonical name; `anira_context_config` with the device descriptors and the log scalars; `anira_contract` as the tagged handle behind `anira::Contract = std::variant<Hard, Async>`, `hard_set_wait_ratio` included; `anira_job_options` with its frame-invariant setters; the extension registry keyed `(kind, version)` with `anira_ext_entry` as its one row, `anira_registered_ext_kinds`, the consumed-or-fail walk; `anira_model_config_from_json`/`anira_context_config_from_json`/`anira_contract_from_json` with the v2 auto-upgrade (`ANIRA_SUCCESS_UPGRADED`, one warning per process, `take_legacy_contract`). Internally a translator from the new handles to v2's `InferenceConfig`/`CoreConfig`/`HostConfig`, so the v2 engine runs unchanged behind the v3 configuration; the v2 classes stay public and exported through this milestone, `include/anira/anira.h` stays the v2 C++ umbrella, and `USE_*`/`ANIRA_VERSION` stay PUBLIC because the v2 headers still need them. The deprecated aliases (`InferenceConfig`, `CoreConfig`, `Core`, `JsonConfigLoader`) are not a v3 deliverable; they become the constructor shims of `compat/v2.hpp` in M2.
...
- Browser WebGPU as an explicitly asynchronous JS backend -- an `anira_backend_desc` tail slot `process_async` with a completion callback driven by `anira_inference_thread_execute` from an unblocked Worker pump, or a JSPI-linked second module variant sharing the header, with `anira_em_context_config_set_js_webgpu` reserved for the `GPUDevice` import; the Context borrows the browser's `GPUDevice` through Emscripten's WebGPU bindings instead of linking Dawn, so that device is always `ANIRA_OWNERSHIP_BORROWED` and the Dawn revision assertion of section 4 does not apply there; `WgpuBuffer` tensors are `GPUBuffer`s the page's own compute passes write; the TS package exposes `allocate_input`/`allocate_output` beside the tickets it already has for that case; and the browser joins section 1's table only after the M4 functional-probe suite has run there, like every other platform column.
...
- Per-class inference-thread groups: a context that requests a scheduling class other than the default is served by its own thread group, the default group staying RealTime so 3.0.0 clients are unaffected -- `anira_context_config_set_thread_class` plus a `thread_class` key in `context.json`. This is what removes both the real-time default and the head-of-line blocking of a long job in front of a short one.
...
- `-Werror=function-effects` widened to `src/capi/` once `InferenceManager`/`Core`/`SessionElement`, `thl::core::RingBuffer` and a moodycamel wrapper are annotated.
...
- **Engine and provider**: done. Two independent enums, `anira_engine` and `anira_provider`, replace the packed `anira_backend`, because a provider name means the same thing across engines -- CUDA is CUDA whether ONNX Runtime or LibTorch runs it -- which the packed form hid; the pair travels as one item only as the Tier-2 `anira_backend_id`. Both are pinned at M1, and `anira_stage_ctx` grows from 56 to 64 bytes for it, pre-freeze, recorded as 64 in `abi/layout-3.txt`. `anira_backend_id` itself lands in `abi/context.h` at M2 with its first consumers; M1 pins only `ANIRA_STRUCT_BACKEND_ID = 11`.
- **Custom engines**: done. A custom engine is an object, `anira_custom_engine_create(desc, &engine, err)`, added to a pipeline by name, `anira_pipeline_add_engine(pipe, engine_id, engine, err)`, never by slot number; the name must be reverse-URI, the same reservation third-party extension kinds already make, so anira's own short engine names can never collide, and adding one name twice in a pipeline is `ANIRA_ERROR_INVALID_STATE` rather than a silent replace. The name belongs to the pipeline and the object is the engine: the pool shares a prepared model between handlers that run the same object on an equal variant, whichever pipelines they come from (instances of one plugin that add one object share models as the built-in engines do; two objects never share), so the name only resolves a model entry. Wherever the pair travels a custom engine is `ANIRA_ENGINE_NONE` with its id, which is why `anira_backend_id` and `anira_plan_info` both carry `engine_id` as a string. `anira_pipeline_set_custom_backend`, `ANIRA_BACKEND_CUSTOM` and the C++ `stage::CustomBackend` are gone: a custom engine is part of the inference stage, one more implementation its candidates resolve to, and its `process` runs in `ANIRA_PHASE_INFERENCE`; the Emscripten twin is `anira_em_pipeline_register_js_engine`, which passes the id the JS engine needs to find its own model row.
...
- **GPU CI hardware** for M4: which contexts run the functional-probe suite. Because the M4 surface is additive, its absence cannot break the M6 freeze -- it leaves enum values unconsumed on CI builds. Recommendation: ship the probe cache as a fixture so CPU-only CI exercises the planner against a recorded registry.
