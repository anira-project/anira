...
/* abi/enums.h */
#define ANIRA_MAX_RANK 8
#define ANIRA_DYNAMIC   ((int64_t)-1)                    /* legal only on a Buffer-role Time axis */
...
#define ANIRA_ANCHOR_FIRST_STREAMED 0xffffffffu               /* v2 HostConfig::k_first_streamable, as uint32_t */
...
class ModelConfig {
    ...
    std::string to_json() const;  std::optional<Contract> take_legacy_contract();
};
```
...
/* The typed twins: void* const* where the entries above take float* const*. The element type of each slot is the stream dtype
   declared on the Hard contract (anira_contract_hard_set_stream_dtype, F32 when unset): the ring beneath the slot holds it, the
...
9. Every declaration carries one thread tag from the fixed vocabulary below, `[callback-safe]` as an additive property where it applies, and `ANIRA_NONBLOCKING` where the body is RT. [`tools/abi/scan_header.py` rejects an untagged declaration]
...
12. No struct-by-value parameter or return anywhere; no 64-bit integer in an `ANIRA_NONBLOCKING` declaration (allowlist: `anira_now_ns` and the GPU tensor factories `anira_tensor_init_vulkan`, `_init_opaque_fd`, `_init_wgpu_buffer`, `_init_dmabuf`, whose parameters are vendor handles and byte offsets at their wire width -- nothing that produces a `VkBuffer` runs in JS; `scan_header.py` carries the list); timeouts and deadlines are `double` milliseconds; indices and sentinels are `uint32_t`; sample and element counts are `size_t`. [`scan_header.py` lint]
...
/* abi/build_info.h (configured by anira/cmake/version.cmake) */
...
Upper-case v2 strings are accepted on the JSON upgrade path (section 8). Enum values, status codes and `ANIRA_ABI_VERSION` reach TS as `web/src/abi/enums.ts`, generated by `tools/abi/scan_header.py` from `abi/enums.h`, `abi/status.h` and `abi/version.h`.

### Thread tags

Every declaration carries one tag; `[callback-safe]` is added where it applies. The vocabulary is CLAP's (`clap/include/clap/plugin.h:41-110`, `ext/thread-check.h:37-41`: "functions marked with [audio-thread] ARE NOT CONCURRENT"), with CLAP's `[driver-thread]` renamed `[driver-thread]` because anira drives frames as well as samples and the tag never named the medium, extended by the roles anira has and CLAP does not:
...
- `[drain-thread]` -- the log sink's thread for RT records.
...
foreach(_prop COMPILE_DEFINITIONS INTERFACE_COMPILE_DEFINITIONS)   # one compiled log ceiling in every build type
    get_target_property(_defs tanh_core ${_prop})
    list(FILTER _defs EXCLUDE REGEX "THL_LOG_COMPILED_MAX_LEVEL")
    set_target_properties(tanh_core PROPERTIES ${_prop} "${_defs};THL_LOG_COMPILED_MAX_LEVEL=4")
endforeach()
...
|---|---|---|---|
| `web/src/abi/enums.ts` | `abi/enums.h`, `abi/status.h`, `abi/version.h`: enum values, status codes, `ANIRA_ABI_VERSION`, struct ids | `tools/abi/scan_header.py` | diff against the committed file |
| `web/src/abi/exports_wasm.txt`, `abi/symbols-<major>.txt` | every `ANIRA_API` declaration of `abi/*.h`, `abi/draft/*.h`, `anira_em.h` | `scan_header.py` | the link (presence); `AniraWeb.d.ts` export names diffed against `symbols-<major>.txt` ∪ `symbols-draft.txt` ∪ `anira_em_*` (extras, until a `wasm-objdump` branch lands in tanh-tooling) |
...
`anira.hpp` keeps the spelling `anira::JsonConfigLoader::model(path)`, `::machine(path)` and `::contract(path)` as static functions returning `anira::ModelConfig`, `anira::MachineConfig` and `anira::Contract`; they are `ModelConfig::from_file`, `MachineConfig::from_file` and the contract's file constructor under the name the sketches use, and they throw `anira::Error` (or return `anira::Result`) on `ANIRA_FAILED`. Nothing about JSON is exported from libanira except the C functions above.
...
| `tensor_shape[].input_shape` / `output_shape` (universal entry) | `inputs[].axes` / `outputs[].axes` | trailing axis `time` (v2 lays channel `i` at `[i * size, (i + 1) * size)`), the axis whose extent equals the channel count `channel`, every other axis `any` |
| `tensor_shape[]` entry with `inference_backend` | -- | one canonical spec per tensor in v3; an entry that differs from the universal one fails the upgrade by name (`ANIRA_ERROR_JSON`) |
| `processing_spec.preprocess_input_channels[i]` / `postprocess_output_channels[i]` | the `channel` extent of `inputs[i]` / `outputs[i]` | |
| `processing_spec.preprocess_input_size[i]` / `postprocess_output_size[i]` | `inputs[i].window` / `outputs[i].window` with `min == max`; `context` = time extent minus the size (the receptive-field rule) | `0` = `"role": "static"` |
| `processing_spec.internal_model_latency[i]` | `outputs[i].latency` | |
| `num_parallel_processors` | `max_instances` | |
...
| `warm_up` (count) | legacy contract `hard.warmup: {"fixed": ...}` | `ANIRA_WARMUP_FIXED` |
...
anira_contract* c;
anira_contract_create_hard(max_frames_count, sample_rate, &c, &err);
...
|---|---|---|
| `ANIRA_ABI_MAJOR`, `ANIRA_ABI_MINOR`, packed as `ANIRA_ABI_VERSION` | `abi/version.h`, hand-written; CMake reads the two lines with `file(STRINGS)` -- one source | the binary promise |
| `ANIRA_VERSION_MAJOR/MINOR/PATCH/STRING`, `ANIRA_MAKE_VERSION` | `abi/build_info.h`, configured from git by `cmake/version.cmake` (prerelease-safe `project()` version, full describe string kept) | release identity; semver major equals ABI major by policy from 3.0.0 |
| `SOVERSION ${ANIRA_ABI_MAJOR}`, `MACHO_COMPATIBILITY_VERSION ${ANIRA_ABI_MAJOR}.${ANIRA_ABI_MINOR}`, `OUTPUT_NAME anira-${ANIRA_ABI_MAJOR}` on PE | `cmake/abi-version.cmake` (today `SOVERSION ${PROJECT_VERSION_MAJOR}`, `CMakeLists.txt:291`) | `libanira.so.3`, `libanira.3.dylib` (dyld refuses a library older than the one a client linked), `anira-3.dll` + `anira-3.lib`; Debian runtime package `libanira3` |
...
- **Tooling**: `tools/abi/scan_header.py` producing `web/src/abi/enums.ts`, `exports_wasm.txt` and both symbol lists, and linting thread tags, the 64-bit rule and the descriptor field order; `abi/layout-0.txt` committed.
...
- **Gates online**: 3 (layout table, native) and 4 (per-file C11 compile, strict isolation). Effort beyond the configuration layer itself: 2.0 person-days.
...
- **Gates**: none new; gate 1 fails the milestone unless `ANIRA_ABI_MINOR` is bumped for the appended `abi/ticket.h` names -- the first exercise of the append rule. Effort beyond the Async work: 1.5 person-days.
...
|---|---|---|---|---|
| 1 | `anira_symbol_baseline` | the real export table (`nm -D` / `nm -gU` / `dumpbin`, through `_tanh_exports` copied into `anira/cmake/abi-symbols.cmake`) against `abi/symbols-<major>.txt` ∪ `abi/symbols-draft.txt`: a promised name missing fails, a name outside both lists fails, the draft list is tolerated in both directions, an added promised name fails unless `ANIRA_ABI_MINOR(HEAD) > ANIRA_ABI_MINOR(last tag)`; on PE additionally "`dumpbin /exports` lists only `anira_*`" | every native shared leg of `build_test`; on Wasm the link against `exports_wasm.txt` is the presence check and the `--emit-tsd` `.d.ts` export diff in `build_web` is the extras check | M2 |
...
- tanh-tooling v0.1.6 re-pinned by anira and tanh-lib together, with a tanh-lib CMake option setting `THL_LOG_COMPILED_MAX_LEVEL` replacing the property edit.
...
1. **M0 tag name**: `v2.3.0` with the no-binary-promise preamble (recommended) or `v3.0.0-alpha.0`. `alpha.0` renames `on_tag.yml` artefacts, the backends-release vocabulary and every "v2 parity" reference the day before the tag, and pulls `cmake/version.cmake` into M0.
...
7. **tanh-tooling v0.1.6 after v3.0.0** (recommended) or bundled with M2. The M1-M3 window is the scarce resource and the anira-local scripts total about eighty lines.
