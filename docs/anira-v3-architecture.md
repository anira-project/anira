...
typedef union anira_memory_handle {                        /* 24 bytes; every arm typeless; vendor typedefs replaced by their wire width */
    ...
    struct { ANIRA_PTR(void, buffer); uint64_t offset; }                      wgpu;      /* WGPUBuffer; same process, the Context's Dawn device */
...
size_t       anira_context_drain_log(anira_context*);         /* [thread-safe, !audio-thread]; ANIRA_LOG_DRAIN_MANUAL */
uint32_t     anira_context_num_inference_threads(const anira_context*);   /* [thread-safe]; the pool behind this handle */
...
if (prev_rects.size() < num_hands) {                                 // the gate: application logic
    write_letterbox(palm_in, frame);                                 // the app's pass on the Context's queue
