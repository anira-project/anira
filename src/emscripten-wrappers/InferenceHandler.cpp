...
int inferencehandler_get_inference_backend(uintptr_t ptr) {
    return static_cast<int>(
        reinterpret_cast<anira::InferenceHandler*>(ptr)->get_inference_backend());
}
...
void inferencehandler_prepare(uintptr_t ptr, uintptr_t host_config_ptr) {
    ...
    emscripten_log(EM_LOG_CONSOLE,
                   "[InferenceHandler] prepare called with buffer_size=%.2f, sample_rate=%.2f",
                   host_config->m_buffer_size,
                   host_config->m_sample_rate);
    reinterpret_cast<anira::InferenceHandler*>(ptr)->prepare(*host_config);
}

EMSCRIPTEN_KEEPALIVE
void inferencehandler_prepare_with_latency(uintptr_t ptr,
...
EMSCRIPTEN_KEEPALIVE
void inferencehandler_prepare_with_latency_vector(uintptr_t ptr,
                                                  uintptr_t host_config_ptr,
                                                  uintptr_t latency_vector_ptr,
    ...
    for (size_t i = 0; i < latency_count; ++i) { latency_vector.push_back(latencies[i]); }
    reinterpret_cast<anira::InferenceHandler*>(ptr)->prepare(
        *reinterpret_cast<anira::HostConfig*>(host_config_ptr),
        latency_vector);
}
...
size_t inferencehandler_process(uintptr_t ptr,
    ...
    return reinterpret_cast<anira::InferenceHandler*>(ptr)->process(data,
                                                                    num_samples,
                                                                    tensor_index);
}
...
size_t inferencehandler_process_separate(uintptr_t ptr,
    ...
    return reinterpret_cast<anira::InferenceHandler*>(ptr)->process(input_data,
                                                                    num_input_samples,
                                                                    output_data,
                                                                    num_output_samples,
                                                                    tensor_index);
...
uintptr_t inferencehandler_process_multi(uintptr_t ptr,
    ...
    size_t* result = reinterpret_cast<anira::InferenceHandler*>(ptr)->process(input_data,
                                                                              num_input_samples,
                                                                              output_data,
                                                                              num_output_samples);
...
void inferencehandler_push_data(uintptr_t ptr,
                                ...
                                size_t tensor_index) {
    const float* const* input_data = reinterpret_cast<const float* const*>(input_ptr);
    reinterpret_cast<anira::InferenceHandler*>(ptr)->push_data(input_data,
                                                               num_samples,
                                                               tensor_index);
}
...
size_t inferencehandler_pop_data(uintptr_t ptr,
                                 ...
                                 size_t tensor_index) {
    float* const* output_data = reinterpret_cast<float* const*>(output_ptr);
    return reinterpret_cast<anira::InferenceHandler*>(ptr)->pop_data(output_data,
                                                                     num_samples,
                                                                     tensor_index);
}
...
EMSCRIPTEN_KEEPALIVE
size_t inferencehandler_pop_data_blocking(uintptr_t ptr,
                                          uintptr_t output_ptr,
                                          size_t num_samples,
                                          double wait_ms,
                                          size_t tensor_index) {
...
uintptr_t inferencehandler_pop_data_multi(uintptr_t ptr,
    ...
    size_t* result =
        reinterpret_cast<anira::InferenceHandler*>(ptr)->pop_data(output_data, num_output_samples);
    return reinterpret_cast<uintptr_t>(result);
}

// No allocations, but not fully real-time safe due to potential blocking (semaphore wait)
EMSCRIPTEN_KEEPALIVE
uintptr_t inferencehandler_pop_data_multi_blocking(uintptr_t ptr,
                                                   uintptr_t output_ptr,
                                                   uintptr_t num_samples_ptr,
                                                   double wait_ms) {
    float* const* const* output_data = reinterpret_cast<float* const* const*>(output_ptr);
    ...
    auto now = std::chrono::steady_clock::now();
    auto wait_until = now + std::chrono::milliseconds(static_cast<long long>(wait_ms));
    size_t* result = reinterpret_cast<anira::InferenceHandler*>(ptr)->pop_data(output_data,
                                                                               num_output_samples,
                                                                               wait_until);
    return reinterpret_cast<uintptr_t>(result);
}
