...
void inferenceconfig_log_debug(uintptr_t ptr) {
    ...
    emscripten_log(EM_LOG_CONSOLE,
                   "  - preprocess_input_size[0]: %zu",
                   config->get_preprocess_input_size()[0]);
    emscripten_log(EM_LOG_CONSOLE,
                   "  - postprocess_output_size[0]: %zu",
                   config->get_postprocess_output_size()[0]);
    ...
    emscripten_log(EM_LOG_CONSOLE,
                   "  - num_parallel_processors: %zu",
                   config->m_num_parallel_processors);
}

// ------ ModelData C API ----

EMSCRIPTEN_KEEPALIVE
uintptr_t modeldata_create(uintptr_t data_ptr,
                           size_t size,
                           int backend,
                           const char* name,
                           bool flag) {
    return reinterpret_cast<uintptr_t>(new anira::ModelData(reinterpret_cast<void*>(data_ptr),
                                                            size,
                                                            to_backend(backend),
                                                            name,
                                                            flag));
...
uintptr_t modeldata_create_from_path(const char* model_path, int backend) {
    return reinterpret_cast<uintptr_t>(new anira::ModelData(model_path, to_backend(backend)));
}

EMSCRIPTEN_KEEPALIVE
uintptr_t modeldata_create_with_function(const char* model_path,
                                         int backend,
                                         const char* model_function,
                                         bool is_binary) {
    return reinterpret_cast<uintptr_t>(
...
bool modeldata_equals(uintptr_t ptr, uintptr_t other_ptr) {
    return *reinterpret_cast<anira::ModelData*>(ptr) ==
           *reinterpret_cast<anira::ModelData*>(other_ptr);
}
...
EMSCRIPTEN_KEEPALIVE
uintptr_t tensorshape_create(uintptr_t input_shape_ptr,
                             size_t input_count,
                             uintptr_t output_shape_ptr,
                             size_t output_count) {
    ...
    return reinterpret_cast<uintptr_t>(new anira::TensorShape(*input_shapes, *output_shapes));
}
...
uintptr_t tensorshape_get_input_shape(uintptr_t ptr) {
    return reinterpret_cast<uintptr_t>(
        &reinterpret_cast<anira::TensorShape*>(ptr)->m_tensor_input_shape);
}

EMSCRIPTEN_KEEPALIVE
uintptr_t tensorshape_get_output_shape(uintptr_t ptr) {
    return reinterpret_cast<uintptr_t>(
        &reinterpret_cast<anira::TensorShape*>(ptr)->m_tensor_output_shape);
}

EMSCRIPTEN_KEEPALIVE
bool tensorshape_equals(uintptr_t ptr, uintptr_t other_ptr) {
    return *reinterpret_cast<anira::TensorShape*>(ptr) ==
           *reinterpret_cast<anira::TensorShape*>(other_ptr);
}
...
EMSCRIPTEN_KEEPALIVE
uintptr_t processingspec_create_with_channels(uintptr_t preprocess_channels_ptr,
                                              size_t pre_ch_count,
                                              uintptr_t postprocess_channels_ptr,
                                              size_t post_ch_count) {
...
EMSCRIPTEN_KEEPALIVE
uintptr_t processingspec_create_full(uintptr_t preprocess_ch_ptr,
                                     size_t pre_ch_count,
                                     uintptr_t postprocess_ch_ptr,
                                     size_t post_ch_count,
                                     uintptr_t preprocess_size_ptr,
                                     size_t pre_size_count,
                                     uintptr_t postprocess_size_ptr,
                                     size_t post_size_count) {
    ...
    return reinterpret_cast<uintptr_t>(new anira::ProcessingSpec(preprocess_ch,
                                                                 postprocess_ch,
                                                                 preprocess_size,
                                                                 postprocess_size));
}
...
EMSCRIPTEN_KEEPALIVE
uintptr_t processingspec_create_full_with_latency(uintptr_t preprocess_ch_ptr,
                                                  size_t pre_ch_count,
                                                  uintptr_t postprocess_ch_ptr,
                                                  size_t post_ch_count,
                                                  uintptr_t preprocess_size_ptr,
                                                  ...
                                                  uintptr_t postprocess_size_ptr,
                                                  size_t post_size_count,
                                                  uintptr_t internal_latency_ptr,
                                                  size_t internal_latency_count) {
    auto& preprocess_ch = *reinterpret_cast<std::vector<size_t>*>(preprocess_ch_ptr);
    auto& postprocess_ch = *reinterpret_cast<std::vector<size_t>*>(postprocess_ch_ptr);
    auto& preprocess_size = *reinterpret_cast<std::vector<size_t>*>(preprocess_size_ptr);
    auto& postprocess_size = *reinterpret_cast<std::vector<size_t>*>(postprocess_size_ptr);
...
bool processingspec_equals(uintptr_t ptr, uintptr_t other_ptr) {
    return *reinterpret_cast<anira::ProcessingSpec*>(ptr) ==
           *reinterpret_cast<anira::ProcessingSpec*>(other_ptr);
}
...
EMSCRIPTEN_KEEPALIVE
uintptr_t inferenceconfig_create_full(uintptr_t model_data_ptrs,
                                      size_t model_count,
                                      uintptr_t tensor_shape_ptrs,
                                      size_t tensor_count,
    ...
    return reinterpret_cast<uintptr_t>(new anira::InferenceConfig(models,
                                                                  shapes,
                                                                  spec,
                                                                  max_inference_time,
                                                                  warm_up,
...
EMSCRIPTEN_KEEPALIVE
uintptr_t inferenceconfig_create_auto_spec(uintptr_t model_data_ptrs,
                                           size_t model_count,
                                           uintptr_t tensor_shape_ptrs,
                                           size_t tensor_count,
                                           float max_inference_time,
...
EMSCRIPTEN_KEEPALIVE
void inferenceconfig_get_model_path(uintptr_t ptr,
                                    int backend,
                                    char* out_buffer,
                                    size_t buffer_size) {
    const std::string& path =
        reinterpret_cast<anira::InferenceConfig*>(ptr)->get_model_path(to_backend(backend));
...
size_t inferenceconfig_get_tensor_input_size(uintptr_t ptr, size_t tensor_index) {
    const std::vector<size_t>& sizes =
        reinterpret_cast<anira::InferenceConfig*>(ptr)->get_tensor_input_size();
...
size_t inferenceconfig_get_tensor_output_size(uintptr_t ptr, size_t tensor_index) {
    const std::vector<size_t>& sizes =
        reinterpret_cast<anira::InferenceConfig*>(ptr)->get_tensor_output_size();
    return tensor_index < sizes.size() ? sizes[tensor_index] : 0;
}
...
size_t inferenceconfig_get_preprocess_input_channels(uintptr_t ptr, size_t tensor_index) {
    const std::vector<size_t>& channels =
        reinterpret_cast<anira::InferenceConfig*>(ptr)->get_preprocess_input_channels();
...
size_t inferenceconfig_get_postprocess_output_channels(uintptr_t ptr, size_t tensor_index) {
    const std::vector<size_t>& channels =
        reinterpret_cast<anira::InferenceConfig*>(ptr)->get_postprocess_output_channels();
    return tensor_index < channels.size() ? channels[tensor_index] : 0;
}
...
size_t inferenceconfig_get_preprocess_input_size(uintptr_t ptr, size_t tensor_index) {
    const std::vector<size_t>& sizes =
        reinterpret_cast<anira::InferenceConfig*>(ptr)->get_preprocess_input_size();
    return tensor_index < sizes.size() ? sizes[tensor_index] : 0;
}
...
size_t inferenceconfig_get_postprocess_output_size(uintptr_t ptr, size_t tensor_index) {
    const std::vector<size_t>& sizes =
        reinterpret_cast<anira::InferenceConfig*>(ptr)->get_postprocess_output_size();
    return tensor_index < sizes.size() ? sizes[tensor_index] : 0;
}

EMSCRIPTEN_KEEPALIVE
size_t inferenceconfig_get_internal_model_latency(uintptr_t ptr, size_t tensor_index) {
    const std::vector<size_t>& latencies =
        reinterpret_cast<anira::InferenceConfig*>(ptr)->get_internal_model_latency();
...
bool inferenceconfig_equals(uintptr_t ptr, uintptr_t other_ptr) {
    return *reinterpret_cast<anira::InferenceConfig*>(ptr) ==
           *reinterpret_cast<anira::InferenceConfig*>(other_ptr);
}
...
uintptr_t inferenceconfig_get_tensor_input_shape(uintptr_t ptr) {
    const anira::TensorShapeList& shape =
        reinterpret_cast<anira::InferenceConfig*>(ptr)->get_tensor_input_shape();
...
uintptr_t inferenceconfig_get_tensor_output_shape(uintptr_t ptr) {
    const anira::TensorShapeList& shape =
        reinterpret_cast<anira::InferenceConfig*>(ptr)->get_tensor_output_shape();
    return reinterpret_cast<uintptr_t>(&shape);
}
...
uintptr_t inferenceconfig_get_tensor_input_shape_for_backend(uintptr_t ptr, int backend) {
    const anira::TensorShapeList& shape =
        reinterpret_cast<anira::InferenceConfig*>(ptr)->get_tensor_input_shape(to_backend(backend));
    return reinterpret_cast<uintptr_t>(&shape);
}
...
uintptr_t inferenceconfig_get_tensor_shape(uintptr_t ptr, int backend) {
    const anira::TensorShape& shape =
        reinterpret_cast<anira::InferenceConfig*>(ptr)->get_tensor_shape(to_backend(backend));
    return reinterpret_cast<uintptr_t>(&shape);
}
