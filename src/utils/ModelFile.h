/*
 * The two things every engine adapter shares on its load path: the model-file pre-check
 * that runs before an engine sees a path, and the one message shape every engine failure
...
/// Formats an engine failure: "<engine>: <where>: <text>" while the three fit the
...
/// normalised form (the path every later message of that engine carries). Throws
