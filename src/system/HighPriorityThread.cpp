...
void HighPriorityThread::elevate_priority(std::thread::native_handle_type thread_native_handle, bool is_main_process) {
    ...
    if (is_main_process) {
        if (!SetPriorityClass(GetCurrentProcess(), REALTIME_PRIORITY_CLASS)) {
           LOG_ERROR << "[ERROR] Failed to set real-time priority for process. Error: " << GetLastError() << std::endl;
        }
    }
    ...
    for (int priority : priorities) {
        if (SetThreadPriority(thread_native_handle, priority)) {
            ...
            LOG_ERROR << "[ERROR] Failed to set thread priority for Thread. Current priority: " << priority << std::endl;
        }
    }
#elif __linux__
    int ret;
    ...
    if (!is_main_process) {
        ...
        if(ret != 0) {
            LOG_ERROR << "[ERROR] Failed to get Thread scheduling policy and params : " << errno << std::endl;\
        }
        if (attr_inheritsched != PTHREAD_EXPLICIT_SCHED) {
            LOG_ERROR << "[ERROR] Thread scheduling policy is not PTHREAD_EXPLICIT_SCHED. Possibly thread attributes get inherited from the main process." << std::endl;
        }
        pthread_attr_destroy(&thread_attr);
    }
    ...
    if(ret != 0) {
        LOG_ERROR << "[ERROR] Failed to set Thread scheduling policy to SCHED_FIFO and increase the sched_priority to " << sch_params.sched_priority << ". Error : " << errno << std::endl;
        LOG_INFO << "[WARNING] Give rtprio privileges to the user by adding the user to the realtime/audio group. Or run the application as root." << std::endl;
        LOG_INFO << "[WARNING] Instead, trying to set increased nice value for SCHED_OTHER..." << std::endl;

        ret = setpriority(PRIO_PROCESS, 0, -10);
        if(ret != 0) {
            LOG_ERROR << "[ERROR] Failed to set increased nice value. Error : " << errno << std::endl;
            LOG_INFO << "[WARNING] Using default nice value: " << getpriority(PRIO_PROCESS, 0) << std::endl;
        }
    }
    ...
    if (ret != 0) {
        LOG_ERROR << "[ERROR] Failed to set Thread QOS class to QOS_CLASS_USER_INTERACTIVE. Error : " << ret << std::endl;
    } else {
        return;
    }

    LOG_ERROR << "[ERROR] Failed to set Thread QOS class and relative priority. Error: " << ret << std::endl;

    qos_class_t qos_class;
    int relative_priority;
    pthread_get_qos_class_np(pthread_self(), &qos_class, &relative_priority);

    LOG_INFO << "[WARNING] Fallback to default QOS class and relative priority: " << qos_class << " " << relative_priority << std::endl;
    return;
#endif
}
