...
void InferenceHandler::process(float* const* data, size_t num_samples, size_t tensor_index) {
    ...
    std::vector<size_t> tensor_num_samples(std::max(num_input_tensors, num_output_tensors), 0);

    // Set the input data at the specified tensor_index
    if (tensor_index < num_input_tensors) {
        input_tensor_ptrs[tensor_index] = data;
        tensor_num_samples[tensor_index] = num_samples;
    }

    // Set the output data at the specified tensor_index
    if (tensor_index < num_output_tensors) {
        output_tensor_ptrs[tensor_index] = data;
        if (tensor_index < tensor_num_samples.size()) {
            tensor_num_samples[tensor_index] = num_samples;
        }
    }
    ...
    m_inference_manager.process(input_3d, tensor_num_samples.data(), output_3d, tensor_num_samples.data());
}

void InferenceHandler::process(const float* const* const* input_data, size_t* num_input_samples, float* const* const* output_data, size_t* num_output_samples) {
    m_inference_manager.process(input_data, num_input_samples, output_data, num_output_samples);
}
...
void InferenceHandler::pop_data(float* const* const* output_data, size_t* num_output_samples) {
    m_inference_manager.pop_data(output_data, num_output_samples);
}
...
std::vector<int> InferenceHandler::get_latency() {
