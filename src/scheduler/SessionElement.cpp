...
void SessionElement::prepare(HostAudioConfig new_config) {
    m_send_buffer.initialize_with_positions(m_inference_config.m_num_audio_channels[Input], (size_t) new_config.m_host_sample_rate * 50); // TODO find appropriate size dynamically
    m_receive_buffer.initialize_with_positions(m_inference_config.m_num_audio_channels[Output], (size_t) new_config.m_host_sample_rate * 50); // TODO find appropriate size dynamically
    ...
    float structs_per_buffer = std::ceil((float) new_config.m_host_buffer_size / (float) new_samples_needed_for_inference);
