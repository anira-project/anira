...
void SessionElement::prepare(HostAudioConfig new_config) {
    ...
    float structs_per_buffer = std::ceil((float) new_config.m_host_buffer_size / (float) m_inference_config.m_output_sizes[m_inference_config.m_index_audio_data[1]]);
    float structs_per_max_inference_time = std::ceil((float) max_inference_time_in_samples / (float) m_inference_config.m_output_sizes[m_inference_config.m_index_audio_data[1]]);
    ...
    for (int i = 0; i < n_structs; ++i) {
        m_inference_queue.emplace_back(std::make_unique<ThreadSafeStruct>(m_inference_config.m_input_sizes[m_inference_config.m_index_audio_data[0]], m_inference_config.m_output_sizes[m_inference_config.m_index_audio_data[1]]));
    }
