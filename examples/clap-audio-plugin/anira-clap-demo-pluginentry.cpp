...
uint32_t clap_get_plugin_count(const clap_plugin_factory* f) {
    return 1;
}
...
static const clap_plugin* clap_create_plugin(const clap_plugin_factory* f,
                                             const clap_host* host,
                                             const char* plugin_id) {
    if (strcmp(plugin_id, AniraClapPluginExample::m_desc.id)) {
        std::cout << "Warning: CLAP asked for plugin_id '" << plugin_id
                  << "' and clap-saw-demo ID is '" << AniraClapPluginExample::m_desc.id << "'"
                  << std::endl;
        return nullptr;
    }
