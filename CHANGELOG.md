...
- **A missed block zeroed the request array of the multi Hard entries.** `num_out` of `anira_handler_process_f32_multi`, `pop_data_f32_multi` and their `_wait` twins is both the request and the result, and the scheduler's miss path wrote 0 into it, so a host that keeps one array across callbacks asked for 0 samples from the first miss on and was answered `ANIRA_OK` with nothing popped while the rings filled. The C entries now hand the scheduler a scratch copy: `num_out` is written only for a delivered block (each count the samples written: the request, clamped to the tensor's size for a Static output) and is left as the caller set it on `ANIRA_MISSED` and on every failure, which the registry now states for all four entries; `ANIRA_MISSED` carries the miss. The single forms are unchanged (their `delivered` is a pure out-parameter, 0 on a miss and on a failure), as is the 2.x `InferenceHandler`. Test: `AbiPrepare.AReusedRequestArraySurvivesAMiss`.
