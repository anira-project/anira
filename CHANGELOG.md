...
- ExecuTorch desktop builds no longer fail on Linux distributions without Debian's multiarch layout (Fedora, Arch, …): the prebuilt `ExecuTorchTargets.cmake` bakes the build machine's absolute `/usr/lib/aarch64-linux-gnu/libm.so` into the XNNPACK targets' `INTERFACE_LINK_LIBRARIES`, which Ninja rejected as a missing input ("needed by 'libanira.so', missing and no known rule to make it") and which leaked a bogus `-L`/rpath entry into the link line. `_anira_sanitize_executorch_targets` now rewrites such absolute Linux system-library paths to plain library names (`m` → `-lm`), as it already did for the macOS SDK `.tbd`/`.framework` paths.
