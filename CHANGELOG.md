...
- Two merge-queue legs that had never run on `v3`: under RealtimeSanitizer the recording pre/post processor of `test_BackendSwitch` appended to a vector inside the `ANIRA_NONBLOCKING` process call (it now appends into reserved capacity), and under ASan+UBSan the C link probe `anira_abi_link` did not link on the runners, because a C link driver does not add UBSan's C++ runtime that an instrumented `libanira` references (the probe links through the C++ driver when `ANIRA_WITH_UBSAN` is on; its source and compile stay C). `AbiPrepare.ThePlanReportIsLoggedAtInfo` asserts its records under `ENABLE_LOGGING` only, like every other record assertion.
