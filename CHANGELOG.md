...
- Symbol policy, in one CMake function (`anira_apply_symbol_policy()`): hidden visibility on every platform with `ANIRA_API` as the export allowlist, `-fno-gnu-unique` under GCC, one section per function/variable with `--gc-sections`/`-dead_strip`/`/OPT:REF` on the shared library, and an export list pinning `libanira`'s table to namespace `anira` (`cmake/anira.map`, `cmake/anira.exports`). A static anira localizes the bundled `libtanh_core.a` on its consumer's link. The `anira_exports` CTest (`cmake/check-exports.cmake`) fails on any non-`anira::` export of the shared library, any engine symbol exported from the plugin-shaped module, or an `STB_GNU_UNIQUE` symbol (on PE, which cannot interpose, the dllexports LibTorch's headers and the LiteRT/TFLite archives force are tolerated). The CLAP example shows the plugin side — hidden visibility, dead-code stripping, an export list pinned to `clap_entry` (`cmake/clap.map`/`clap.exports`) — and the unload module carries the same (`test/unload/module.map`/`.exports`).
...
- The export-decoration header is `anira/system/AniraExports.h` (`AniraWinExports.h` forwards, deprecated); `ANIRA_BUILDING` replaces `ANIRA_EXPORTS`, `ANIRA_STATIC` replaces `ANIRA_STATIC_DEFINE` (old spellings honoured).
