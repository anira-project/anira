...
- **ExecuTorch backend** (`ANIRA_WITH_EXECUTORCH`, enabled by default; static-only): runs `.pte` programs exported with `torch.export` through the new `ExecuTorchProcessor` (`InferenceBackend::EXECUTORCH`, XNNPACK-delegated, file and buffer loading, JSON config support), with prebuilt static libraries from anira-project/backends for desktop and Android/iOS, `EXECUTORCH` entries in the bundled model configs, benchmarks and examples, and a `minimal-executorch` example. Mobile CI keeps it off until validated on-device; in fully static desktop builds it coexists with static LiteRT (pre-isolated archives in backends v2.3.0), while with the legacy TFLite backend and on mobile it stays auto-disabled in fully static builds.

### Changed

- The ExecuTorch backend now consumes one merged `libexecutorch.a` per platform (`executorch.lib` + a whole-archived `executorch_registrations.lib` on Windows) from anira-project/backends, with the kernel/backend registrations pre-linked into the archive, and links it like the other static backends. This replaces the ExecuTorch CMake package wiring (`find_package(executorch)`, the per-target force-load options, the imported-target sanitizing and the isolated object target for `ExecuTorchProcessor.cpp`, which now gets the ExecuTorch include dirs per-source); the CMake ≥ 3.24 requirement on desktop and the `xnnpack-microkernels-prod` whole-archive special case are gone. Installed trees no longer carry `lib/cmake/ExecuTorch`, and `aniraConfig.cmake` no longer re-resolves it. The per-backend CMake wiring moved out of the top-level `CMakeLists.txt` into `cmake/backends/<engine>.cmake`, each exposing `anira_setup_<engine>(<target>)` (fetch, processor source, `USE_<ENGINE>`, include/link dirs, linking); `cmake/AniraBackends.cmake` keeps the download/resolve machinery and shared link helpers. **Requires the matching backends release** (the desktop ExecuTorch archives are now flat `include/` + `lib/` like the mobile ones).
...
- Default backends release tag bumped from v2.1.1 to v2.4.0 (ExecuTorch 1.3.1 as one merged archive with pre-linked registrations, pre-isolated desktop static LiteRT archives).
