...
- Scheduler, `allow_smaller_buffers`: `prepare()` no longer walks every smaller buffer size. The old sweep counted a `float` down from the greatest relative buffer size and recomputed the latency and struct count at every step -- millions of iterations for large buffers, each costing up to a hop's worth of work, and above 2^24 samples the counter could no longer decrement and the sweep never terminated. The worst case is now evaluated only at the buffer sizes where it can occur (the peaks of the buffer-quantized inference time, the blocking flip points and the boundaries of the inference count per buffer), which keeps the result bit-identical to the exhaustive walk while the cost scales with the square root of the inference time in samples instead of with the buffer size. Tests: `SessionElementSmallerBufferSweep`, plus a small corpus of configurations pinned from the old sweep (`SessionElementSmallerBufferCorpus`).

- Scheduler: `least_common_multiple()` overflowed `int` once the host block times a stream hop exceeded 2^31 (a frame-sized block on the streaming path), silently dropping the buffer adaptation and truncating the inference count. It now divides before multiplying in 64 bits. The whole-sample cases of the buffer adaptation and of the per-buffer inference count also take their closed forms (`hop - gcd`, and `floor(buffer / hop)` plus one unless the remainder divides the hop) instead of walking the alignment cycle. Test: `SessionElementLcmOverflow`.
