...
- CI: merges are gated by GitHub's merge queue — the full matrix tier of every workflow runs on the exact merged result (`merge_group`) before anything lands, with `build_test result` as the required status.
...
>>>>>>> 506b5e5 (test: drop the upstream-covered ring-buffer test; merge two duplicate pairs)
