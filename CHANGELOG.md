...
- anira pins tanh-lib's compile-time log ceiling (`THL_LOG_COMPILED_MAX_LEVEL`) to 4 on its private copy in every build type: the runtime level is the only filter, and Warning and Info records exist in Release builds (before, a Release anira compiled everything below Error out).
