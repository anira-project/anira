...
- Optional diff-based clang-tidy pre-push hook, installed from tanh-tooling's new `hooks` family (`hooks/tanh/pre-push`; tanh-tooling pinned at v0.2.2 — its cmake family is unchanged at 0.1.5, so the fetched tanh-lib's modules still agree). Activation is per clone (git does not version hooks): `ln -sf ../../hooks/tanh/pre-push .git/hooks/pre-push`, or chain from a `core.hooksPath` pre-push; see `contributing.rst`.
