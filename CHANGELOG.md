...
- **Breaking:** anira logs through tanh-lib's `thl::Logger` instead of its own `std::cout`/`std::cerr` stream macros. Every record carries an `anira.<component>` group (`anira.context`, `anira.scheduler`, `anira.config`, `anira.system`, `anira.backend.<name>`, `anira.web`), `ContextConfig::m_log_level` is applied as `thl::Logger`'s runtime level (and still forwarded to the backends), and anira never calls `set_config` — the host owns the sinks. Migration: output now goes to tanh-lib's default sinks (the platform log — `os_log` on Apple platforms, `logcat` on Android — or stdout/stderr elsewhere); hosts that want a console on macOS/iOS or a callback configure `thl::Logger` themselves. The `LOG_DEBUG`/`LOG_INFO`/`LOG_WARNING`/`LOG_ERROR` macros in `anira/utils/Logger.h` are replaced by printf-style `ANIRA_LOG_*` and real-time-safe `ANIRA_LOG_RT_*` (aliases of tanh-lib's new `THL_LOG_*` / `THL_LOG_RT_*`); `ANIRA_WITH_LOGGING=OFF` still compiles every call out. tanh-lib also filters at compile time — Release builds compile in Error records only — so a more verbose `m_log_level` in a Release build affects the backends' output but not anira's own.
- Logging from real-time paths is real-time safe: the sites reachable from `InferenceHandler::process()`/`push_data()`/`pop_data()` and from the inference threads (queue-full drops, missing-samples warnings) use `thl::Logger::rt` — a stack-formatted record pushed into a bounded lock-free queue, delivered by a process-global drain thread that anira starts with the first session (unless the host disabled tanh-lib's rt path) and stops only in `Context::shutdown()` — never on a session's behalf, since a host logging through `thl::Logger` shares it. On WebAssembly the queue is drained manually via the new `_anira_drain_log()` export / `drainAniraLog()` helper. On Windows, `tanh_core.dll` (installed to `bin/`) is a load-time dependency of `anira.dll`. The RTSan CI job now runs without suppressions (`.github/rtsan-suppressions.supp` is gone) and builds tanh-lib with `TANH_WITH_RTSAN` so its own `[[clang::nonblocking]]` annotations are checked too.
...
- tanh-lib pin bumped from v0.0.3 to the `THL_LOG_*` macros / teardown-safe `rt::stop()` commit (tanh-lib PR #17; to become v0.0.4).
