...
- anira sets tanh-lib's new `TANH_LOG_COMPILED_MAX_LEVEL` option to 4 for its private copy in every build type: the runtime level is the only filter, and Warning and Info records exist in Release builds (before, a Release anira compiled everything below Error out). tanh-lib re-pinned to the commit that carries the option (tanh-lab/tanh-lib#37: the configurable platform identity, per-record flags and the drop count on the record, which the 3.x runtime's log projection uses).
