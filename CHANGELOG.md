...
- The Windows legs of the `build_test` workflow now run the test suite: `ctest` is invoked with `-C Release`, which the Visual Studio generator needs — without it the gtest-discovered tests were not listed and the legs passed on "No tests were found". `test/install` copies a consumer's runtime DLLs next to its executable (`$<TARGET_RUNTIME_DLLS>`), as an application must on Windows: the loader searches System32 before `PATH`, and Windows ships its own `onnxruntime.dll` (Windows ML, ORT 1.17) there, so an executable that calls ONNX Runtime with only `<prefix>/bin` on `PATH` binds to the wrong runtime.
- `anira_exports` CTest (`cmake/check-exports.cmake`, registered with the unload tests): fails when the shared `libanira` exports any symbol outside namespace `anira`, when the plugin-shaped unload module exports a backend-runtime symbol (`Ort*`, `torch::`/`c10::`, `executorch::`, `xnn_*`, `TfLite*`/`LiteRt*`, …), or when either defines an `STB_GNU_UNIQUE` symbol — via `nm` on Linux/macOS and `dumpbin /exports` on Windows (where PE binds imports by module and cannot interpose, so the `c10::`/`caffe2::` exports LibTorch's headers force on every DLL and the `LiteRt*`/`TfLite*` ones the static archives' dllexport-built objects force on a plugin are tolerated). The plugin-shaped unload module carries the export list a plugin should have (`test/unload/module.map` / `module.exports`).
...
- **Breaking:** no public header of anira includes an engine header any more. `OnnxRuntimeProcessor`, `LibtorchProcessor`, `TFLiteProcessor` and `LiteRtProcessor` keep their engine state behind the same named pimpl (`struct Instance`, defined only in the `.cpp`) that `ExecuTorchProcessor` already used; the pattern is documented once, on `BackendBase`. A translation unit that includes `<anira/anira.h>` now compiles with anira's include directory alone, which the new `anira_header_isolation` CTest enforces. Migration: code that relied on `onnxruntime_cxx_api.h`, `torch/torch.h`, `tensorflow/lite/c_api.h` or the `litert/c/*.h` headers coming in through an anira header includes them itself (and, see below, links the matching `anira::<engine>` target).
- **Breaking:** the engines are explicit targets and anira links them `PRIVATE`. Every enabled engine is an imported target `anira::<engine>` — `anira::onnxruntime`, `anira::tflite`, `anira::litert`, `anira::libtorch`, `anira::executorch` — defined by one function (`cmake/aniraBackendHelpers.cmake`) both in the build tree and in the installed package (`aniraBackendTargets.cmake`, generated at configure time and included by `aniraConfig.cmake`), with identical usage requirements: the engine's include directories and definitions, and the file anira links — a `SHARED IMPORTED` target for a shared engine, an `INTERFACE IMPORTED` one carrying the archive linked on demand and hidden (`-Wl,-load_hidden` on macOS, `--exclude-libs` on ELF) plus its system libraries for a static one, and wrappers around `find_package(Torch)` / `find_package(executorch)` for LibTorch and ExecuTorch. `target_link_libraries(x anira::anira)` now gives `x` anira's headers and `USE_*` definitions and nothing of any engine — no include directory, no link directory, no bare `onnxruntime`/`tensorflowlite_c`/`LiteRt` on the link line (a static anira still hands its archives to the consumer's link as `$<LINK_ONLY:anira::<engine>>`). A consumer whose own code calls an engine links `anira::<engine>` in addition and gets the very file anira uses, so the process holds one copy of the engine. The installed package puts each engine's headers under `include/anira-backends/<engine>/` (LibTorch's `TorchConfig.cmake` is re-pointed there) instead of flat into `include/`, which is what makes "you need `anira::<engine>`" true at compile time; the libraries stay in `lib/`. `test/install` gained an engine-calling consumer (executable and plugin-shaped module, whose export table must carry no engine symbol) and a negative `try_compile` asserting that the engine header is unreachable through `anira::anira` alone; both run in the `InstallConsumer` CTest and in `build_install`. The minimal-inference examples link the engine targets. `TORCH_CXX_FLAGS` is a `PUBLIC` compile option of `anira` (it decides anira's own `std::` ABI) instead of a global `CMAKE_CXX_FLAGS` append. `anira_target_link_static_backend()` and the `ANIRA_<ENGINE>_LINKAGE` variables are gone. Migration: a consumer that included engine headers through anira's include path, or linked `onnxruntime`/`tensorflowlite_c`/`LiteRt`/`${TORCH_LIBRARIES}`/the ExecuTorch targets by name relying on anira's link directory, adds the one line `target_link_libraries(your_target PRIVATE anira::<engine>)`.
- **Breaking** (Windows install layout): DLLs install to `bin/`, import libraries and static archives to `lib/`, as GNUInstallDirs and vcpkg/Conan do — `anira.dll`, the engine DLLs (ONNX Runtime, LiteRT, TFLite, LibTorch) and tanh-lib's `tanh_core.dll` now all sit in one directory next to any installed executable, where the loader finds them. Previously `anira.dll` and the engine DLLs went to `lib/` while `tanh_core.dll` went to `bin/`, so a consumer had to put both directories on `PATH`. The installed `anira::<engine>` targets and LibTorch's package files point at the new locations. Migration: `PATH` needs `<prefix>/bin` only; anything that copied DLLs out of `<prefix>/lib` copies from `<prefix>/bin`.
- The build-tree CMake modules under `cmake/` are named lowercase-hyphen throughout, like tanh-lib's: `backends.cmake` → `backends.cmake`, `validate-options.cmake` → `validate-options.cmake`, `check-exports.cmake` → `check-exports.cmake`, `detect-emscripten.cmake` → `detect-emscripten.cmake`, `build-wasm.cmake` → `build-wasm.cmake`, and `test/install/run-install-test.cmake` → `run-install-test.cmake`. The files shipped in the installed package keep CMake's `anira<Thing>.cmake` form (`aniraConfig`, `aniraTargets`, `aniraBackendHelpers`, `aniraBackendTargets`). Only relevant if you `include()` one of these files directly.
...
- **Breaking:** anira logs through tanh-lib's `thl::Logger` instead of its own `std::cout`/`std::cerr` stream macros. Every record carries an `anira.<component>` group (`anira.context`, `anira.scheduler`, `anira.config`, `anira.system`, `anira.backend.<name>`, `anira.web`), the configured level is applied as `thl::Logger`'s runtime level (and still forwarded to the backends), and anira never calls `set_config` — the host owns the sinks. Migration: output now goes to tanh-lib's default sinks (the platform log — `os_log` on Apple platforms, `logcat` on Android — or stdout/stderr elsewhere); hosts that want a console on macOS/iOS or a callback configure `thl::Logger` themselves. The `LOG_DEBUG`/`LOG_INFO`/`LOG_WARNING`/`LOG_ERROR` macros in `anira/utils/Logger.h` are replaced by printf-style `ANIRA_LOG_*` (tanh-lib's `THL_LOG_*`) and real-time-safe `ANIRA_LOG_RT_*`; `ANIRA_WITH_LOGGING=OFF` still compiles every call out. tanh-lib also filters at compile time — Release builds compile in Error records only — so a more verbose level in a Release build affects the backends' output but not anira's own.
- **Breaking:** `ContextConfig::m_log_level` became `ContextConfig::m_log` (`anira::LogConfig`): `m_level` (the old field), plus `m_drain` (`LogDrain::Thread` | `Manual`), `m_queue_capacity` (default 512) and `m_drain_interval_ms` (default 10). The three-argument `ContextConfig(num_threads, wait_strategy, log_level)` constructor is unchanged. JSON: a `context_config.log` block with `level`, `drain`, `queue_capacity`, `drain_interval_ms`; the old top-level `log_level` key is still accepted. Migration: `config.m_log_level = x` → `config.m_log.m_level = x`.
- Logging from real-time paths is real-time safe: the sites reachable from `InferenceHandler::process()`/`push_data()`/`pop_data()` and from the inference threads (queue-full drops, missing-samples warnings, backend exceptions) log into a lock-free queue owned by the context (`thl::Logger::rt::Queue`, sized by `LogConfig::m_queue_capacity`). With `LogDrain::Thread` a context-owned low-priority thread (`thl::core::ThreadPriority::Low`) drains it exactly while sessions exist — started with the first, stopped and joined with the last (outside the lifecycle lock, so a host log callback may call back into the context) and by `Context::shutdown()` — so nothing survives the last handler and a plugin host may unload the library right after. With `LogDrain::Manual` no thread exists and the host pumps the new `InferenceHandler::drain_log()` / `Context::drain_log()` (WebAssembly: the `_anira_drain_log()` export / `drainAniraLog()` helper; Manual is forced there). Either way the queue is flushed when the last handler is released. The RTSan CI job runs without suppressions (`.github/rtsan-suppressions.supp` is gone) and builds tanh-lib with `TANH_WITH_RTSAN` so its own `[[clang::nonblocking]]` annotations are checked too. On Windows, `tanh_core.dll` (installed to `bin/`) is a load-time dependency of `anira.dll`.
- `anira::HighPriorityThread` is deprecated: the platform priority handling moved to tanh-lib's `thl::core::Thread` (with `ThreadPriority::{Background, Low, Normal, High, RealTime}`), which `InferenceThread` now composes instead of inheriting `HighPriorityThread`. The class stays for one minor release as a thin wrapper; `elevate_priority()` now applies to the calling thread. Migration: `thl::core::Thread::set_current_priority(thl::core::ThreadPriority::RealTime)`. On Linux, anira no longer sets the process-wide default thread attributes (`pthread_setattr_default_np`) — a side effect on every thread the host created afterwards.
...
- tanh-lib pin bumped from v0.0.3 to the commit of tanh-lib PR #17 (`THL_LOG_*` macros, `thl::core::Thread`, owned `thl::Logger::rt::Queue`/`DrainThread`, no implicit drain thread; to become v0.1.0).
...
- `USE_ANIRA_WEB` was defined in every build, desktop included: the generator expression tested the literal string `EMSDK_VERSION` instead of the variable. The only code that reads it, `OnnxRuntimeProcessor`, therefore created its `Ort::Env` with ONNX Runtime's global thread pools (the WebAssembly configuration) on desktop too. Only Emscripten builds define it now; desktop builds create the plain per-session environment as intended.
- Installing anira on hosts where `CMAKE_INSTALL_LIBDIR` is `lib64` (Fedora) produced a package whose `find_package(anira)` failed whenever LibTorch or ExecuTorch was part of it: their bundled CMake packages (`Caffe2Targets.cmake`, `TorchConfig.cmake`, `ExecuTorchTargets.cmake`) hardwire their libraries to `<prefix>/lib/`, while anira installs them into `lib64` next to `libanira`. anira now installs patched copies of those package files (`<prefix>/lib64/...`) on such hosts; the libraries stay in the libdir. The `InstallConsumer` CTest also puts `lib64` on the runtime search path.
