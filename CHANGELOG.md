...
### Added
- Backends are fetched per engine from anira-project/backends' per-backend releases (`<engine>-v<upstream version>[-<n>]`): `ANIRA_<ENGINE>_TAG` overrides one engine's release tag, the built-in per-engine defaults live in `cmake/backends.cmake` (`_anira_engine_tag`; every engine now defaults to its per-engine release; LiteRT moves to 2.2.0 and ONNX Runtime to 1.30.0 — whose `-gpu` archives carry the WebGPU EP + Dawn), and the legacy umbrella releases (`v2.4.0` and older) stay reachable through `ANIRA_BACKENDS_VERSION`; the live integrity check fetches one release's metadata per distinct tag. `ANIRA_<ENGINE>_VARIANT` (`gpu` | `cuda`) selects a GPU variant archive (`<lib>-<ver>-<OS>-<arch>-<variant>-<linkage>`; iOS `-iOS-<variant>-xcframework`, Android `-Android-<variant>-<linkage>`); the default archives stay CPU-only. An onnxruntime `-gpu` package carries the WebGPU EP built against an external Dawn shipped in the same archive: it is exposed as `anira::webgpu_dawn` (also from the installed package), with `ANIRA_ONNXRUNTIME_HAS_WEBGPU` and `ANIRA_ONNXRUNTIME_DAWN_VERSION` (the ORT-pinned Dawn revision) for the Context's revision assertion.
