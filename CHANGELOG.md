...
- The test suite reads its WAV fixtures through tanh-lib's new header-only `thl::core::read_wav` (Apache-2.0 core component) instead of the in-tree `test/WavReader.h` (#91). The old reader only understood 32-bit float mono files; the tanh decoder handles PCM 8/16/24/32-bit, float 32/64-bit, `WAVE_FORMAT_EXTENSIBLE` and multichannel files, and reports a reason on failure instead of printing to stdout. tanh-lib is pinned to the commit that adds it until the next tanh-lib release.
