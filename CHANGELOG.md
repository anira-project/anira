...
- CI: merges are gated by GitHub's merge queue — the full matrix tier of every workflow runs on the exact merged result (`merge_group`) before anything lands, with `build_test result` as the required status; a weekly scheduled run executes the full tier as a backstop. Extended sanitizers (asan/tsan/lsan) await their CMake wiring and are tracked in `docs/ci-overhaul.md`.
