...
- `desktop-install-{debug,release}` developer presets: `ANIRA_WITH_INSTALL=ON` with the default backend set, building into `build/desktop/Install/<Config>` for `cmake --install` (the README's build-from-source recipe now passes `-DANIRA_WITH_INSTALL=ON`, without which `cmake --install` installed nothing of anira's).

### Changed

- **Breaking:** the latency calculation moves out of `SessionElement` into the new `LatencyCalculator` (`anira/scheduler/LatencyCalculator.h`), in closed form and in floating point (#66). The buffer-adaptation delay is Rath & Geier's `P - gcd(b, P)` ("Minimum required delay for realtime block size adaptation", LAC 2026) instead of the PortAudio-style loop over every host block below `lcm(b, P)`; the inference-caused latency is the exact worst case of the scheduler's queue, `Λ = (q-1)/q + max_m [ρ·d_m - m·n]` hops with `ρ = p/q` the host block in hops, `d_m = ⌈(m+1)τ - β⌉` the callbacks until the (m+1)-th batch is collected, `n` the parallel processors; the steady-state number of inference slots is `S = max_m [⌈(d_m+1)·ρ⌉ - m·n]` instead of the `N·(1 + ⌈κ⌉)` heuristic, and the session allocates `2·S` so that the in-flight inferences a wait-free `reset()` strands until their workers finish never starve the fresh schedule; `allow_smaller_buffers` takes the worst case over every smaller block from the breakpoints of the ceiling alone. `SessionElement` loses its `calculate_*`, `max_num_inferences` and gcd/lcm helpers (public "for testing" before); `sync_latencies` is a static of the calculator. Reported latencies are unchanged for whole-sample host blocks. Slot counts follow `2·S` (4 instead of 2 for a 2048-sample block at 48 kHz with a 40 ms model, 16 instead of 12 for a 4096-sample block at 96 kHz with a 1024-sample hop, 2 as before whenever the blocking wait covers the inference); the previous count left no reset headroom for most configurations and none at all for a smaller host block that needs more slots than the maximum one. `allow_smaller_buffers` latencies move by one sample for fractional per-stream blocks (the flexible-host adaptation `(H-1)/H` is applied consistently) and the send rings of such hosts are `⌈b⌉ + P - 1` instead of `2⌈b⌉ + P - gcd`. A configuration whose inference work exceeds the pool, `κ = T·f_s/(1000·R) ≥ n` hop periods of work per hop, is reported by `LatencyCalculator::is_feasible()` and logs a warning from `prepare()`; its values describe one host block on an idle pool. The derivation is in `docs/sphinx/latency.rst`. Tests: `LatencyCalculatorAdaptation` (the paper's table and the brute-force loop over every pair below 96), `LatencyCalculatorArithmetic`, `LatencyCalculatorClosedForm` (the closed forms against an exact event simulation of the scheduler over ~1000 configurations, and the breakpoint evaluation of the smaller-block grid against a brute force over the whole grid).
...
- Tests: the macOS `build_test` legs failed intermittently in the post-build test discovery with `ParseTestList.cmake:96 ... Syntax error: value, object or array expected` (`test_utils` on 2026-08-31, `test_scheduler` on 2026-09-01, `test_utils` and `test_backends` on the v2.3.0 tag run) while the same commit passed in the next run: CMake 4.4.0 (Homebrew, on the macOS runner image) writes every target's `--gtest_list_tests` JSON to one shared file — the per-target hash is taken from an empty name (CMake issue 27972, fixed in 4.4.1) — so the four test binaries linking in parallel truncated each other's list between the writer's exit and CMake's read. The desktop test binaries use `DISCOVERY_MODE PRE_TEST`: ctest enumerates them one after another when it starts, on any CMake version.
- `least_common_multiple` overflowed `int` once the host block times a hop exceeded 2^31 (v3 architecture notes, M0 item 5): `LatencyCalculator::least_common_multiple` is `int64_t` and divides before multiplying.
- The `allow_smaller_buffers` sweep counted a float down from the greatest relative buffer size, recomputing every latency at every step (millions of iterations for large blocks) and, above 2^24, decrementing a value it could not represent so that it stalled (v3 architecture notes, M0 item 5): the worst case over smaller blocks is evaluated at the breakpoints of the ceiling function only, a handful of points for any block size.
- Tests: `OneSidedStreamingStandalone.GeneratorBlockingDeadlineUsesReference` failed intermittently on loaded runners (Windows clang on 2026-09-01, macOS x86_64 under Rosetta on 2026-09-02) with "the pop gave up after 2 us": the first pop of a block had starved (tolerated by design) and the retry reused the count array that `process()` had written 0 into, so it asked for 0 samples and returned at once. The retry sets its demand again; after a starvation the output may lag the demand by the starved samples until the handler's catch-up discards them, so a delivered sample is checked against the range of parameters that lag allows (sample-exact without starvation); `GeneratorStarvedBlockingPopIsRetriedWithFullDemand` exercises the path deterministically (a 500 ms stall of the first inference against a 341 ms deadline); and the Doxygen of the array overloads of `process()`/`pop_data()` now says that the count array is written back.
- The send ring of a stream whose host block is a fractional number of samples was sized for a constant block (`P - gcd(⌈b⌉, P)` leftover), up to `gcd - 1` samples short when the host alternates between `⌊b⌋` and `⌈b⌉` samples; it reserves `P - 1` now.
