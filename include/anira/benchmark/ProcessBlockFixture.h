 ...
 * processing contexts
 *
 * The ProcessBlockFixture provides a specialized Google Benchmark fixture for measuring the
 * performance of neural network inference operations in real-time audio processing scenarios. It
 * manages test setup, data preparation, timing measurements, and cleanup for benchmarking different
 * inference backends, model configurations, and buffer sizes.
 *
 * This fixture supports:
 * - Performance testing across different inference backends (ONNX, LibTorch, TensorFlow Lite,
 * Custom)
...
public:
     ...
     * @param sleep_after_repetition Whether to sleep after each repetition to allow system cooldown
     * (default: true)
     */
    void initialize_repetition(const InferenceConfig& inference_config,
                               const HostConfig& host_config,
                               const InferenceBackend& inference_backend,
                               bool sleep_after_repetition = true);
     ...
     */
    void interation_step(const std::chrono::steady_clock::time_point& start,
...
private:
    ...
    bool m_sleep_after_repetition = true;  ///< Whether to sleep after repetitions for system
                                           ///< cooldown
    int m_iteration = 0;                   ///< Current iteration number within a repetition
    std::chrono::duration<double, std::milli> m_runtime_last_repetition =
