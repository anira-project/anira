...
struct ANIRA_API ModelData {
     ...
     */
    bool operator!=(const ModelData& other) const { return !(*this == other); }
};
...
struct ANIRA_API TensorShape {
    TensorShapeList m_tensor_input_shape;   ///< List of input tensor shapes (each shape is a vector
                                            ///< of dimensions)
...
struct ANIRA_API ProcessingSpec {
    std::vector<size_t> m_preprocess_input_channels;    ///< Number of input channels for each input
                                                        ///< tensor
    std::vector<size_t> m_postprocess_output_channels;  ///< Number of output channels for each
                                                        ///< output tensor
    std::vector<size_t> m_preprocess_input_size;  ///< Samples count required for preprocessing for
                                                  ///< each input tensor (0 = non-streamable)
    std::vector<size_t> m_postprocess_output_size;  ///< Samples count after the postprocessing for
                                                    ///< each output tensor (0 = non-streamable)
    std::vector<size_t> m_internal_model_latency;   ///< Internal latency in samples for each output
                                                    ///< tensor
    std::vector<size_t> m_tensor_input_size;        ///< Total size (elements) of each input tensor
                                                    ///< (computed from shape)
    std::vector<size_t> m_tensor_output_size;       ///< Total size (elements) of each output tensor
                                                    ///< (computed from shape)
     ...
     * @param preprocess_input_size Samples count required for preprocessing for each input tensor
     * (0 = non-streamable)
     * @param postprocess_output_size Samples count after the postprocessing for each output tensor
     * (0 = non-streamable)
     ...
     * @return true if any members are not equal, false otherwise
     */
    bool operator!=(const ProcessingSpec& other) const { return !(*this == other); }
};
...
public:
     ...
     * @param preprocess_input_size New samples count required for preprocessing (0 =
     * non-streamable)
     */
    ...
    std::vector<TensorShape> m_tensor_shape;  ///< Tensor shape configurations for inputs and
                                              ///< outputs
    ProcessingSpec m_processing_spec;         ///< Processing specification for preprocessing and
                                              ///< postprocessing
    ...
    bool operator==(const InferenceConfig& other) const {
        return m_model_data == other.m_model_data && m_tensor_shape == other.m_tensor_shape &&
               std::abs(m_max_inference_time - other.m_max_inference_time) < 1e-6 &&
               m_processing_spec == other.m_processing_spec && m_warm_up == other.m_warm_up &&
