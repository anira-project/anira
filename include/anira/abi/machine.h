/*
 * anira/abi/machine.h -- generated from abi/anira.yml by tools/abi/gen.py.
 * Do not edit: change the registry and run `python3 tools/abi/gen.py --repo . --write`.
 */
#ifndef ANIRA_ABI_MACHINE_H
#define ANIRA_ABI_MACHINE_H

/**
 * @file machine.h
 * @brief The machine handle over the core, its capabilities and the probe, the steady clock, and the shutdown family.
 *
 * anira_machine is a refcounted handle over the immortal core, one core per copy of anira (a
 * shared library has one, every static embedding its own). anira_machine_create reconciles the
 * machine config into the core (the first machine's config takes effect whole; later machines
 ...
 * a warning), registers the config's log sink and probes the capabilities. Two machines in one
 ...
 * while a machine or a handler lives. In this pre-release every machine is Host-only: the probe
...
/**
 * @brief Creates a machine over this copy's core: reconciles the config into the core (see the
 * file comment), registers the config's log sink so that it receives every record from
 * here on, applies the config's ANIRA_LOG_FLAG_* switches while the machine lives, and
 ...
 * are coerced with a warning, as the machine file section says.
 * @param config The machine config; copied, destroyable right after.
 ...
 *         machine extension nothing consumes.
 * @par Thread contract
 * [main-thread]
 * @since ABI 0.2
 */
ANIRA_API anira_status ANIRA_CALL anira_machine_create(const anira_machine_config* config,
                                                       anira_machine** out,
                                                       anira_error* err) ANIRA_NOEXCEPT;

/**
 * @brief Drops the user's reference: flushes the real-time log queue when this was the last
 * user of the core, unregisters the machine's log sink and waits for its in-flight
 ...
 * alive). Joins nothing. A destroy issued from inside the machine's own sink is refused
 * with one Error record and nothing happens.
 * @param machine The handle; NULL is a no-op.
 * @par Thread contract
 * [main-thread & !loader-lock]
 * @since ABI 0.2
 */
ANIRA_API void ANIRA_CALL anira_machine_destroy(anira_machine* machine) ANIRA_NOEXCEPT;
 ...
 * @param machine The handle.
 * @param force Nonzero re-runs every rung even where a cached answer exists.
 * @param err Nullable.
 * @return ANIRA_OK, or ANIRA_ERROR_INVALID_ARGUMENT for a NULL machine.
 * @par Thread contract
 * [main-thread]
 * @since ABI 0.2
 */
ANIRA_API anira_status ANIRA_CALL anira_machine_probe(anira_machine* machine,
                                                      anira_bool force,
                                                      anira_error* err) ANIRA_NOEXCEPT;

/**
 * @brief The capabilities the last probe established; machine-owned, valid while the machine
 * is, replaced in place by anira_machine_probe.
 * @param machine The handle.
 * @return The capabilities, or NULL for a NULL machine.
 * @par Thread contract
 * [thread-safe]
 * @since ABI 0.2
 */
ANIRA_API const anira_capabilities* ANIRA_CALL anira_machine_capabilities(const anira_machine* machine)
                                                                          ANIRA_NOEXCEPT;
...
/**
 * @brief The memory domains a tensor may live in on this machine; ANIRA_DOMAIN_HOST alone in
 * this pre-release.
 * @param capabilities The capabilities.
...
/**
 * @brief What this build compiled in, without a machine: every engine with an adapter, on
 * ANIRA_PROVIDER_DEFAULT, in anira_engine order. Whether a backend is usable here is
 * anira_capabilities_backends on a probed machine.
 * @param element_size sizeof(anira_backend_id) of the caller's header, the stride of out.
 * @param count In: the capacity of out in elements; out: the number of backends.
 * @param out Receives the rows at the caller's stride, or NULL to ask for the count only.
...
/**
 * @brief The size in bytes of a tensor's byte image on this machine (section 7): the dense
 * encoding, num_elements times the element size, in this pre-release.
 * @param machine The handle.
 * @param num_elements Elements of the tensor.
 * @param dtype The element type.
 * @return The byte count, 0 for a NULL machine or a dtype without a size.
 * @par Thread contract
 * [thread-safe]
 * @since ABI 0.2
 */
ANIRA_API uint64_t ANIRA_CALL anira_machine_byte_image_bytes(const anira_machine* machine,
                                                             uint64_t num_elements,
                                                             anira_dtype dtype) ANIRA_NOEXCEPT;

/**
 * @brief Delivers the queued real-time records of the core behind this machine to the sinks:
 * the host's pump under ANIRA_LOG_DRAIN_MANUAL. The queue is shared by every machine of
 * the copy, so pumping any one of them drains everything.
 * @param machine The handle.
 * @return The number of records delivered; 0 for a NULL machine.
 * @par Thread contract
 * [thread-safe, !audio-thread]
 * @since ABI 0.2
 */
ANIRA_API size_t ANIRA_CALL anira_machine_drain_log(anira_machine* machine) ANIRA_NOEXCEPT;

/**
 * @brief The size of the inference thread pool serving this machine: the default pool of the
 * copy, which exists while a handler does, so 0 before the first handler and for a
 * machine that brought its own threads.
 * @param machine The handle.
 * @return The pool size; 0 for a NULL machine.
 * @par Thread contract
 * [thread-safe]
 * @since ABI 0.2
 */
ANIRA_API uint32_t ANIRA_CALL anira_machine_num_inference_threads(const anira_machine* machine)
                                                                  ANIRA_NOEXCEPT;
 ...
 * ExitDll). Idempotent, never creates the core, and effective only when no machine and
 ...
 * @return ANIRA_OK (also without a core); ANIRA_ERROR_INVALID_STATE while a machine or a
 *         handler lives.
 * @par Thread contract
 * [main-thread & !loader-lock]
 * @since ABI 0.2
...
/**
 * @brief Frees the core when nothing uses it: no machine, no handler, no pool thread, no
