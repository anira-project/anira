...
public:
    ...
    AudioBuffer(size_t num_channels, size_t size)
        : m_num_channels(num_channels), m_size(size)
    {
        allocate_memory();
        clear();
    }
    ...
    AudioBuffer(T* const* data, size_t num_channels, size_t size, bool copy_data = true)
        : m_num_channels(num_channels), m_size(size)
    {
        if (copy_data) {
            allocate_memory();
            ...
            m_p_channels = new T*[m_num_channels];
            for (size_t i = 0; i < m_num_channels; i++) {
    ...
    AudioBuffer(const AudioBuffer& other)
        : m_num_channels(other.m_num_channels), m_size(other.m_size)
    {
        if (m_num_channels == 0 || m_size == 0) {
            return;
        } else {
            allocate_memory();
            std::memcpy(m_p_data, other.m_p_data, m_num_channels * m_size * sizeof(T));
        }
    }
    ...
    AudioBuffer(AudioBuffer&& other) noexcept
        : m_num_channels(other.m_num_channels), m_size(other.m_size), m_p_data(other.m_p_data), m_p_channels(other.m_p_channels)
    {
    ...
    {
        initialize(other.m_num_channels, other.m_size);
        std::memcpy(m_p_data, other.m_p_data, m_num_channels * m_size * sizeof(T));
        return *this;
    }
    ...
    void reset_from_data(T* const* data, size_t num_channels, size_t size, bool copy_data = true)
    {
        delete[] m_p_data;
        delete[] m_p_channels;
    ...
    void clear()
    {
        for (size_t i = 0; i < m_num_channels * m_size; i++) {
            m_p_data[i] = 0;
        }
    }
    ...
    template <typename U>
    void make_copy_of(const AudioBuffer<U>& other)
    {
        initialize(other.m_num_channels, other.m_size);
        if (typeid(T) == typeid(U)) {
            std::memcpy(m_p_data, other.m_p_data, m_num_channels * m_size * sizeof(T));
        } else {
            const U* p_source_data = other.get_raw_data();
            for (size_t i = 0; i < m_num_channels * m_size; i++) {
...
private:
    ...
    {
        m_p_data = new T[m_num_channels * m_size];
        m_p_channels = new T*[m_num_channels];
        for (size_t i = 0; i < m_num_channels; i++) {
