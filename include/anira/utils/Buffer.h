...
public:
     ...
     */
    Buffer(size_t num_channels, size_t size)
        : m_num_channels(num_channels), m_size(size), m_data(num_channels * size) {
        malloc_channels();
        clear();
    }
    ...
    Buffer(const Buffer& other)
        : m_num_channels(other.m_num_channels), m_size(other.m_size), m_data(other.m_data) {
     ...
     *
     * @note This operation involves memory allocation and should not be called in real-time
     * contexts
     */
