 ...
 *
 * The TensorFlow Lite state lives behind the named pimpl `Instance`, defined only in
 * TFLiteProcessor.cpp: this header includes no engine header (see BackendBase).
 *
...
private:
     ...
     *
     * Opaque here, defined in TFLiteProcessor.cpp. Each Instance owns an independent
     * TensorFlow Lite model, interpreter and tensors. Each instance is used by only
