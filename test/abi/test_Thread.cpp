// anira/abi/thread.h: user-driven inference threads over a machine with num_threads = 0,
...
struct UserThreadMachine {
    UserThreadMachine() {
        EXPECT_EQ(anira_machine_config_create(&m_config, &m_err), ANIRA_OK);
        EXPECT_EQ(anira_machine_config_set_threads(m_config, 0, ANIRA_WAIT_SPIN_BACKOFF), ANIRA_OK);
        EXPECT_EQ(anira_machine_config_set_log_level(m_config, ANIRA_LOG_ERROR), ANIRA_OK);
        EXPECT_EQ(anira_machine_create(m_config, &m_machine, &m_err), ANIRA_OK) << m_err.message;
    }
    ~UserThreadMachine() {
        anira_machine_destroy(m_machine);
        anira_machine_config_destroy(m_config);
    }
    UserThreadMachine(const UserThreadMachine&) = delete;
    UserThreadMachine& operator=(const UserThreadMachine&) = delete;
...
TEST(AbiInferenceThread, NullArgumentsAreRefused) {
    const UserThreadMachine machine;
    ...
    EXPECT_EQ(anira_inference_thread_create(machine.m_machine, nullptr, &err),
...
TEST(AbiInferenceThread, RunLoopOnTheCallersOwnThread) {
    ...
    EXPECT_EQ(anira_inference_thread_has_exited(thread), 0U);
    EXPECT_EQ(Context::release_core_if_idle(), false) << "a thread inside its loop uses the core";
...
TEST(AbiInferenceThread, ServesASessionWithoutAPool) {
    ...
    // Zero auto-pool threads, like the machine: the user owns the threading.
    const ContextConfig context_config(0, WaitStrategy::SpinBackoff, LogLevel::Error);
    ...
    EXPECT_EQ(anira_machine_num_inference_threads(machine.m_machine), 0U);
