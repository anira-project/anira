// Field-by-field comparison of two 2.x InferenceConfigs, with a named message per mismatch
// (the equality operator alone says only "not equal"). Shared by the JSON loader test and
// the 3.x bridge tests.
#ifndef ANIRA_TEST_SUPPORT_INFERENCE_CONFIG_EQ_H
#define ANIRA_TEST_SUPPORT_INFERENCE_CONFIG_EQ_H
...
inline void expect_inference_config_eq(const anira::InferenceConfig& a,
                                       const anira::InferenceConfig& b) {
    ...
    for (size_t i = 0; i < a.m_model_data.size() && i < b.m_model_data.size(); ++i) {
        ...
        if (model_data_a.m_size == model_data_b.m_size) {
    ...
    for (size_t i = 0; i < a.m_tensor_shape.size() && i < b.m_tensor_shape.size(); ++i) {
...
#endif  // ANIRA_TEST_SUPPORT_INFERENCE_CONFIG_EQ_H
